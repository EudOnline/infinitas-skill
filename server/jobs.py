...
DEFAULT_JOB_LEASE_SECONDS = 1800
MAX_JOB_ATTEMPTS = 10
...
def _running_job_is_stale(now: datetime) -> ColumnElement[bool]:
    return and_(
        Job.status == "running",
        or_(
...
def _active_status_clause(status: str, *, now: datetime) -> ColumnElement[bool]:
    if status != "running":
...
def append_job_log(job: Job, *lines: str) -> None:
    existing = job.log or ""
