...
import server.modules.discovery.service as discovery_service
...
@lru_cache(maxsize=1)
def _search_runtime_defaults() -> tuple[dict[str, Any], str, list[str]]:
...
def _search_payload(entries: Sequence[Any], *, scope: str) -> dict[str, Any]:
...
def _snapshot_skill_payloads(items: list[dict[str, Any]], *, limit: int) -> list[dict[str, Any]]:
    runtime, runtime_readiness, workspace_targets = _search_runtime_defaults()
    results = []
