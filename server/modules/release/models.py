...
class Release(Base):
    ...
    __table_args__ = (
        UniqueConstraint("skill_version_id", name="uq_releases_skill_version_id"),
    ...
    manifest_artifact_id: Mapped[int | None] = mapped_column(
    ...
    signature_artifact_id: Mapped[int | None] = mapped_column(
        ForeignKey("artifacts.id", ondelete="SET NULL", use_alter=True),
        nullable=True,
    )
    provenance_artifact_id: Mapped[int | None] = mapped_column(
        ForeignKey("artifacts.id", ondelete="SET NULL", use_alter=True),
        nullable=True,
    )
