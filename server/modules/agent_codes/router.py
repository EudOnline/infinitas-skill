...
from server.modules.agent_codes import service
...
router = APIRouter(prefix="/api/v1", tags=["agent-codes"])
...
@router.post("/agent-codes", response_model=AgentCodeView, status_code=status.HTTP_201_CREATED)
def create_agent_code(
    ...
    try:
        record = service.create_agent_code(
            db,
    ...
    return AgentCodeView.from_model(
        record.spec,
...
@router.get("/agent-codes/{code_id}", response_model=AgentCodeView)
def get_agent_code(
    code_id: int,
    context: AccessContext = Depends(get_current_access_context),
    db: Session = Depends(get_db),
    ...
    try:
        record = service.get_agent_code_or_404(db, code_id)
...
def create_agent_code_draft(
    ...
    principal_id, is_maintainer = _require_principal(context)
    try:
        draft = service.create_agent_code_draft(
            db,
            code_id=code_id,
            actor_principal_id=principal_id,
            is_maintainer=is_maintainer,
...
@router.post(
    "/agent-code-drafts/{draft_id}/seal",
...
def seal_agent_code_draft(
    ...
    principal_id, is_maintainer = _require_principal(context)
    try:
        draft, skill_version = service.seal_agent_code_draft(
            db,
            draft_id=draft_id,
            actor_principal_id=principal_id,
            is_maintainer=is_maintainer,
...
@router.post(
    "/agent-code-versions/{version_id}/releases",
    response_model=ReleaseView,
    status_code=status.HTTP_201_CREATED,
)
def create_agent_code_release(
    version_id: int,
    response: Response,
    context: AccessContext = Depends(get_current_access_context),
    db: Session = Depends(get_db),
    ...
    if created:
        try:
            ...
            db.rollback()
            existing = db.get(release_service.Release, release.id)
