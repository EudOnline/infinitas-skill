...
| server | `uv run infinitas server healthcheck --api-url URL --repo-path PATH --artifact-path PATH --database-url URL [--token TOKEN] [--json]` | maintained | `python3 scripts/server-healthcheck.py --api-url URL --repo-path PATH --artifact-path PATH --database-url URL [--token TOKEN] [--json]` | shim | 2026-06-30 |
| server | `uv run infinitas server backup --repo-path PATH --database-url URL --artifact-path PATH --output-dir PATH [--label LABEL] [--json]` | maintained | `python3 scripts/backup-hosted-registry.py --repo-path PATH --database-url URL --artifact-path PATH --output-dir PATH [--label LABEL] [--json]` | shim | 2026-06-30 |
| server | `uv run infinitas server inspect-state --database-url URL [--limit N] [--max-queued-jobs N] [--max-running-jobs N] [--max-failed-jobs N] [--max-warning-jobs N] [--alert-webhook-url URL] [--alert-fallback-file PATH] [--json]` | maintained | `python3 scripts/inspect-hosted-state.py --database-url URL [--limit N] [--max-queued-jobs N] [--max-running-jobs N] [--max-failed-jobs N] [--max-warning-jobs N] [--alert-webhook-url URL] [--alert-fallback-file PATH] [--json]` | shim | 2026-06-30 |
| server | `uv run infinitas server render-systemd --output-dir PATH --repo-root PATH --python-bin PATH --env-file PATH --backup-output-dir PATH ...` | maintained | `python3 scripts/render-hosted-systemd.py --output-dir PATH --repo-root PATH --python-bin PATH --env-file PATH --backup-output-dir PATH ...` | shim | 2026-06-30 |
| server | `uv run infinitas server prune-backups --backup-root PATH --keep-last N [--json]` | maintained | `python3 scripts/prune-hosted-backups.py --backup-root PATH --keep-last N [--json]` | shim | 2026-06-30 |
| server | `uv run infinitas server worker [--poll-interval SECONDS] [--once] [--limit N]` | maintained | `python3 scripts/run-hosted-worker.py [--poll-interval SECONDS] [--once] [--limit N]` | shim | 2026-06-30 |
