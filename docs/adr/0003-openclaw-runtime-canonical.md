...
At the same time, this repository already has a durable backend for skills, immutable versions, releases, exposures, reviews, access grants, artifacts, and audit records. Replacing that backend is unnecessary for the current migration and would expand scope far beyond the runtime-design problem.
...
- Registry lifecycle records remain authoritative for `skill -> immutable version -> release -> exposure -> review case -> grant/credential -> discovery/install`.
