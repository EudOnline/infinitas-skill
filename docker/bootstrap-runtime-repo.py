...
def main() -> int:
    ...
    bundled_repo_path = Path(
        os.environ.get("INFINITAS_BUNDLED_REPO_PATH") or "/opt/infinitas/bundle"
    )
    repo_lock_path = Path(
        os.environ.get("INFINITAS_SERVER_REPO_LOCK_PATH") or "/srv/infinitas/data/repo.lock"
    )
    artifact_path = Path(
        os.environ.get("INFINITAS_SERVER_ARTIFACT_PATH") or "/srv/infinitas/artifacts"
    )
    ...
    git_user_name = str(
        os.environ.get("INFINITAS_SERVER_GIT_USER_NAME") or "Infinitas Hosted Registry"
    ).strip()
    git_user_email = str(
        os.environ.get("INFINITAS_SERVER_GIT_USER_EMAIL") or "hosted-registry@example.com"
    ).strip()
