...
@contextmanager
def _patched_verification(root: Path, **verify_attestation_kwargs):
    with (
        patch(
            "infinitas_skill.install.distribution_verification.verify_attestation",
            **verify_attestation_kwargs,
        ),
        patch("infinitas_skill.install.distribution_verification.ROOT", root),
    ):
        yield
...
def test_verify_distribution_manifest_rejects_invalid_json(tmp_path: Path):
    ...
    with _patched_verification(tmp_path):
        try:
...
def test_verify_distribution_manifest_rejects_attestation_error(tmp_path: Path):
    ...
    with _patched_verification(tmp_path, side_effect=AttestationError("SSH verification failed")):
        try:
            verify_distribution_manifest("manifest.json", root=tmp_path)
        except DistributionError as exc:
...
def test_verify_distribution_manifest_rejects_name_mismatch(tmp_path: Path):
    ...
    with _patched_verification(tmp_path, return_value=mock_result):
        try:
            verify_distribution_manifest("manifest.json", root=tmp_path)
        except DistributionError as exc:
