...
def assert_policy_check_promotion_returns_expected_json():
    ...
    if cli_payload.get("skill_path") not in {
        str(ACTIVE_SKILL),
        "skills/active/operate-infinitas-skill",
    }:
        fail(
            "expected check-promotion skill_path to reference "
            f"{ACTIVE_SKILL}, got {cli_payload.get('skill_path')!r}"
        )
...
def assert_policy_review_commands_route_through_package_modules():
    ...
    for legacy_module in ["reviewer_rotation_lib", "review_lib"]:
        if modules.get(legacy_module):
            fail(
                f"policy review command still imports legacy {legacy_module} directly from scripts/"
            )
    for module_name in [
        "infinitas_skill.policy.review_commands",
        "infinitas_skill.policy.reviewer_rotation",
