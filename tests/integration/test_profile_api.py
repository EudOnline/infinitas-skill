...
# ═══════════════════════════════════════════════════════════════════════════════
# Task 3: POST /api/v1/profile/writeback
# ═══════════════════════════════════════════════════════════════════════════════


class TestProfileWritebackAuth:
    """Authentication for the writeback endpoint."""
...
class TestProfileWritebackData:
    """Data handling for the writeback endpoint."""

    def test_returns_recorded(self, tmp_path: Path):
        client = _profile_client(tmp_path)
        response = client.post(
            "/api/v1/profile/writeback",
            headers={"Authorization": "Bearer profile-test-token"},
            json={"note": "User searched for X"},
        )
        assert response.status_code == 200
        assert response.json() == {"status": "recorded"}

    def test_creates_audit_event(self, tmp_path: Path):
        client = _profile_client(tmp_path)
        # Get credential id
        profile = client.get(
            "/api/v1/profile/me",
        ...
        client.post(
            "/api/v1/profile/writeback",
            headers={"Authorization": "Bearer profile-test-token"},
            json={"note": "test note", "context": {"key": "value"}},
        )
        ...
        with factory() as session:
            event = session.scalar(
                __import__("sqlalchemy", fromlist=["select"]).select(AuditEvent).where(
                    AuditEvent.aggregate_type == "memory_writeback",
                    AuditEvent.aggregate_id == str(cred_id),
                    AuditEvent.event_type == "memory.writeback",
                )
            )
            assert event is not None
            assert event.actor_ref == "profile-tester"
            payload = json.loads(event.payload_json)
            assert payload["note"] == "test note"
            assert payload["context"] == {"key": "value"}
            assert event.occurred_at is not None

    def test_context_can_be_null(self, tmp_path: Path):
        client = _profile_client(tmp_path)
        response = client.post(
            "/api/v1/profile/writeback",
            headers={"Authorization": "Bearer profile-test-token"},
            json={"note": "no context", "context": None},
        )
        assert response.status_code == 200
        assert response.json() == {"status": "recorded"}

    def test_context_can_be_omitted(self, tmp_path: Path):
        client = _profile_client(tmp_path)
        response = client.post(
            "/api/v1/profile/writeback",
            headers={"Authorization": "Bearer profile-test-token"},
            json={"note": "no context field"},
        )
        assert response.status_code == 200

    def test_uses_credential_id_when_no_principal(self, tmp_path: Path):
        """When principal is None, actor_ref should be str(credential.id)."""
        client = _profile_client(tmp_path)
        # Create a credential with no principal
        from server.db import get_session_factory
        ...
        # Use the service token to post writeback
        response = client.post(
            "/api/v1/profile/writeback",
            headers={"Authorization": f"Bearer {raw}"},
            json={"note": "service note"},
        )
        assert response.status_code == 200
        ...
        with factory() as session:
            ...
            assert event is not None
            assert event.actor_ref == str(cred_id)


# ═══════════════════════════════════════════════════════════════════════════════
# Task 4: PATCH /api/v1/credentials/{credential_id}/policy
# ═══════════════════════════════════════════════════════════════════════════════
