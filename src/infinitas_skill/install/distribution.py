...
def validate_distribution_manifest(payload):
    ...
    if not isinstance(attestation_bundle, dict):
        ...
        if required_formats is not None and (
            not isinstance(required_formats, list) or not required_formats
        ):
            errors.append(
                "attestation_bundle.required_formats must be a non-empty array when present"
            )
        elif isinstance(required_formats, list) and any(
            item not in {"ssh", "ci"} for item in required_formats
        ):
...
def verify_distribution_manifest(manifest_path, root=None, attestation_root=None):
    ...
    try:
        ...
        raise DistributionError(
            f"could not parse distribution manifest {manifest_path}: {exc}"
        ) from exc
    ...
    provenance_path = _resolve_manifest_ref(
        manifest_path,
        attestation_bundle["provenance_path"],
        root=root,
    )
    signature_path = _resolve_manifest_ref(
        manifest_path,
    ...
    if "ci" in required_formats and "ci" not in formats_verified:
        ...
        ci_path = (
            _resolve_manifest_ref(manifest_path, ci_ref, root=root)
            if ci_ref
    ...
    if (
        payload_file_manifest is not None
        or signed_file_manifest is not None
        or payload_build is not None
        or signed_build is not None
    ):
        actual_bundle_metadata = inspect_distribution_bundle(
            bundle_path,
            expected_root=signed_bundle.get("root_dir"),
        )

    comparisons = [
        (
            "skill.name",
            payload["skill"].get("name"),
            provenance.get("skill", {}).get("name"),
        ),
        (
            "skill.version",
    ...
    if payload_file_manifest is not None:
        ...
        if (
            ...
            and normalized_payload_file_manifest != normalized_signed_file_manifest
        ):
            raise DistributionError("file manifest does not match signed attestation payload")
        normalized_actual_file_manifest = _normalize_file_manifest(
            (actual_bundle_metadata or {}).get("file_manifest")
        )
    ...
    if payload_build is not None:
        ...
        if (
            normalized_signed_build is not None
            and normalized_payload_build != normalized_signed_build
        ):
            raise DistributionError("build metadata does not match signed attestation payload")
        normalized_actual_build = _normalize_build(
            (actual_bundle_metadata or {}).get("build"),
    ...
    info.update(
        {
            ...
            "distribution_attestation": payload.get("attestation_bundle", {}).get(
                "provenance_path"
            ),
            "distribution_attestation_signature": payload.get("attestation_bundle", {}).get(
                "signature_path"
            ),
            "distribution_attestation_sha256": payload.get("attestation_bundle", {}).get(
                "provenance_sha256"
            ),
            "distribution_attestation_signature_sha256": payload.get("attestation_bundle", {}).get(
                "signature_sha256"
            ),
...
def _materialize_remote_distribution_source(info):
    ...
    if not bundle_ref or not provenance_ref or not signature_ref:
        raise DistributionError(
            "hosted distribution manifest is missing bundle or attestation references"
        )
...
def build_distribution_manifest_payload(
    ...
    signature_path = provenance_path.with_suffix(
        provenance_path.suffix + (provenance.get("attestation") or {}).get("signature_ext", ".ssig")
    )
    if not signature_path.exists():
        raise DistributionError(f"missing attestation signature: {signature_path}")
