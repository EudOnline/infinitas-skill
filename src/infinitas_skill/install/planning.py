...
def emit_resolve_install_plan_error(exc, *, stderr=None):
    ...
    if conflict:
        ...
        print(
            f"  conflict: {conflict.get('name')}{registry} {conflict.get('version')}", file=stderr
        )
    ...
    if constraints:
        ...
        for entry in constraints:
            ...
            source = (
                f" <= {entry.get('source_name')}@{entry.get('source_version')}"
...
def configure_resolve_install_plan_parser(
    ...
    parser.add_argument(
        ...
        default="install",
        help="Whether to plan an install or sync flow",
    )
...
def build_resolve_install_plan_parser(*, prog: str | None = None) -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog=prog, description="Resolve an install or sync dependency plan"
    )
    return configure_resolve_install_plan_parser(parser)


def parse_resolve_install_plan_args(
...
def configure_check_install_target_parser(
    parser: argparse.ArgumentParser,
) -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        choices=INSTALL_MODES,
        default="install",
        help="Whether to check an install or sync flow",
    )
    parser.add_argument("--json", action="store_true", help="Print machine-readable plan output")
    return parser
...
def build_check_install_target_parser(*, prog: str | None = None) -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog=prog, description="Check whether an install target is dependency-safe"
    )
    return configure_check_install_target_parser(parser)


def parse_check_install_target_args(
    argv: list[str] | None = None, *, prog: str | None = None
) -> argparse.Namespace:
