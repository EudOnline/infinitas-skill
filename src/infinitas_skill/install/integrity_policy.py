"""Installed-skill integrity policy helpers."""
...
def normalize_install_integrity_policy(payload):
    ...
    if "$schema" in payload:
        if not isinstance(schema_ref, str) or not schema_ref.strip():
            raise InstallIntegrityPolicyError(
                "install-integrity policy $schema must be a non-empty string when present"
            )
    ...
    if not isinstance(stale_after_hours, int) or stale_after_hours < 1:
        raise InstallIntegrityPolicyError(
            "install-integrity policy freshness.stale_after_hours must be an integer >= 1"
        )
    ...
    if not isinstance(max_inline_events, int) or max_inline_events < 1:
        raise InstallIntegrityPolicyError(
            "install-integrity policy history.max_inline_events must be an integer >= 1"
        )
    normalized["history"] = {
...
__all__ = [
    ...
    "DEFAULT_NEVER_VERIFIED_POLICY",
    "DEFAULT_MAX_INLINE_EVENTS",
    "default_install_integrity_policy",
    "normalize_install_integrity_policy",
    "load_install_integrity_policy",
]
