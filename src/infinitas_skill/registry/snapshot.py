...
def _load_snapshot_metadata_file(metadata_path: Path) -> JsonDict | None:
    try:
...
def _path_for_output(root: Path, value: object) -> str | None:
...
def _snapshot_sort_key(item: JsonDict) -> tuple[str, str]:
    return (item.get("created_at") or "", item.get("snapshot_id") or "")


def summarize_snapshot(root: Path, payload: object, metadata_path: Path) -> JsonDict | None:
    if not isinstance(payload, dict):
        return None
    ...
    return {
        ...
        "snapshot_root": _path_for_output(root, payload.get("snapshot_root"))
        or _path_for_output(root, str(metadata_path.parent / "registry")),
...
def list_registry_snapshots(root: Path, registry_name: str) -> list[JsonDict]:
...
def snapshot_catalog_summary(root: Path, registry_name: str) -> JsonDict:
...
def load_snapshot_metadata(root: Path, registry_name: str, snapshot_id: str) -> JsonDict | None:
...
def resolve_snapshot_selector(root: Path, registry_name: str, selector: str) -> JsonDict | None:
    ...
    if choice == "latest":
        ...
        choice = latest_id if isinstance(latest_id, str) else ""
    if not choice:
        return None
    return load_snapshot_metadata(root, registry_name, choice)


def default_snapshot_id(*, now: datetime | None = None, source_commit: str | None = None) -> str:
    timestamp = utc_now_iso(now).replace("-", "").replace(":", "")
    suffix = (source_commit or "snapshot")[:12]
    return f"{timestamp}-{suffix}"


def _write_snapshot_metadata(path: Path, payload: JsonDict) -> None:
...
def create_snapshot(
    ...
    *,
    ...
    if normalized_update_policy(reg).get("mode") == "local-only":
        raise ValueError(
            f"registry '{registry_name}' is local-only and cannot produce an external snapshot"
        )

    cache_root = resolve_registry_root(root, reg)
    if cache_root is None or not cache_root.exists():
        raise ValueError(
            f"registry '{registry_name}' cache root is unavailable; "
            "sync it before creating a snapshot"
        )
    ...
    if (
        any(
            refresh_policy.get(key) is not None
            for key in ["interval_hours", "max_cache_age_hours", "stale_policy"]
        )
        and not refresh_state
    ):
        raise ValueError(
            f"registry '{registry_name}' is missing refresh state; "
            "sync it before creating a snapshot"
        )
    ...
    if destination.exists():
        raise ValueError(
            f"snapshot '{chosen_snapshot_id}' already exists for registry '{registry_name}'"
        )
