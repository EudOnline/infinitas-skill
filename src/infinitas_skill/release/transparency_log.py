"""Transparency log helpers used by release-state evaluation."""
...
def normalize_transparency_log_entry(response_payload, *, endpoint, request_payload):
    ...
    response_digest = _require_string(
        response_payload.get("attestation_sha256"), "response.attestation_sha256"
    )
    if response_digest != expected_digest:
        raise TransparencyLogError(
            "transparency log proof digest does not match attestation digest"
        )
    ...
    if proof_digest != expected_digest:
        raise TransparencyLogError(
            "transparency log proof body digest does not match attestation digest"
        )

    inclusion_path = proof.get("inclusion_path")
    if not isinstance(inclusion_path, list) or any(
        not isinstance(item, str) or not item.strip() for item in inclusion_path
    ):
    ...
    return {
        ...
        "log_index": _require_non_negative_int(
            response_payload.get("log_index"), "response.log_index"
        ),
        "integrated_time": _require_string(
            response_payload.get("integrated_time"), "response.integrated_time"
        ),
        "attestation_sha256": expected_digest,
        "proof": {
            "hash_algorithm": _require_string(
                proof.get("hash_algorithm") or "sha256", "proof.hash_algorithm"
            ),
...
def verify_transparency_log_entry(entry_path, provenance_path):
    ...
    if payload.get("kind") != "attestation-transparency-log-entry":
        raise TransparencyLogError(
            "transparency log entry kind must be attestation-transparency-log-entry"
        )
...
def summarize_transparency_log_state(provenance_path, payload=None, root=None):
    ...
    entry_path = resolve_transparency_log_entry_path(
        provenance_path, descriptor=descriptor, root=root
    )
    summary = {
        "mode": mode,
        "required": required,
        "entry_path": _relative_or_absolute(root, entry_path)
        if root is not None
        else str(entry_path),
        "published": False,
        "verified": False,
...
def submit_transparency_log_entry(endpoint, request_payload, timeout_seconds=5):
    ...
    try:
        ...
        raise TransparencyLogError(
            f"transparency log rejected the request: HTTP {exc.code}"
        ) from exc
    except error.URLError as exc:
        reason = exc.reason if hasattr(exc, "reason") else exc
        raise TransparencyLogError(
            f"could not reach transparency log endpoint {endpoint}: {reason}"
        ) from exc
