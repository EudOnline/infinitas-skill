...
def collect_platform_contract_result(
    ...
    *,
    ...
    if doc_sources and profile_sources and profile_sources != doc_sources:
        errors.append(
            f"{profile_path}: contract.sources do not match {path}: {profile_sources!r} != {doc_sources!r}"
        )

    verified_date = payload.get("last_verified")
    profile_last_verified = profile_payload.get("last_verified")
    if (
        isinstance(verified_date, date)
        and profile_last_verified
        and profile_last_verified != verified_date.isoformat()
    ):
        errors.append(
