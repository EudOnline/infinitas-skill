...
load_promotion_policy = review_lib.load_promotion_policy
...
def run_check_promotion(
    *,
    ...
    if not resolved_targets:
        ...
        resolved_targets = [path for path in base.iterdir() if path.is_dir() and (path / '_meta.json').exists()] if base.exists() else []

    reports = [
        collect_skill_report(target, as_active=as_active, root=root, exception_policy=exception_policy)
