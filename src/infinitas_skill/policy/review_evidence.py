...
def load_review_evidence(skill_dir: Path):
    ...
    for index, raw_entry in enumerate(entries):
        ...
        unknown = sorted(
            set(raw_entry)
            - {"source", "source_kind", "source_ref", "reviewer", "decision", "at", "url", "note"}
        )
        if unknown:
            raise ReviewEvidenceError(
        ...
        if normalized_entry["decision"] not in ALLOWED_DECISIONS:
            raise ReviewEvidenceError(
                "review evidence entry has invalid decision "
                f"{normalized_entry['decision']!r}: {entry_path}"
            )
        if _parse_timestamp(normalized_entry["at"]) is None:
            raise ReviewEvidenceError(
                "review evidence entry has invalid timestamp "
                f"{normalized_entry['at']!r}: {entry_path}"
            )
        ...
        for key in ["url", "note"]:
            ...
            if not isinstance(value, str) or not value.strip():
                raise ReviewEvidenceError(
                    "review evidence entry field "
                    f"{key!r} must be a non-empty string when present: {entry_path}"
                )
            normalized_entry[key] = value.strip()
