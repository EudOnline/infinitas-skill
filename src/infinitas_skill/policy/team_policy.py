"""Team policy loading for package-native policy evaluation."""
...
def validate_team_policy(payload):
    ...
    for team_name, raw_team in raw_teams.items():
        ...
        if unknown:
            errors.append(
                f"team-policy teams.{team_name} has unsupported keys: {', '.join(unknown)}"
            )
        ...
        if raw_team.get("delegates") is not None and not isinstance(
            raw_team.get("delegates"), list
        ):
        ...
        if description is not None and not isinstance(description, str):
            errors.append(
                f"team-policy teams.{team_name}.description must be a string when present"
            )
            description = None
...
def load_team_policy(root=ROOT):
    ...
    try:
        ...
        if exc.errors and all(
            error.startswith("missing policy source for domain 'team_policy'")
