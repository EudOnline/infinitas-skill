.PHONY: bootstrap clean-local ci-fast test-fast test-contracts test-full test-markers check-all test-e2e lint-maintained fmt-maintained doctor build-css watch-css typecheck
...
ci-fast: lint-maintained test-fast test-contracts
...
typecheck:
	.venv/bin/mypy src/infinitas_skill server/ --ignore-missing-imports
