...
KNOWN_REGISTRIES = {
    reg.get("name") for reg in load_registry_config(ROOT).get("registries", []) if reg.get("name")
}
...
def validate_canonical_skill(skill_dir: Path) -> int:
    ...
    for error in validate_canonical_payload(payload):
        ...
        errors += 1
    instructions_rel = (
        payload.get("instructions_body", "instructions.md")
...
def validate_meta(skill_dir: Path, namespace_policy=None) -> int:
    ...
    if isinstance(frontmatter_name, str) and frontmatter_name != name:
        fail(
            f"{skill_dir}: frontmatter name {frontmatter_name!r} does not match _meta.json name {name!r}"
        )
        errors += 1
    ...
    for list_key in ["maintainers", "tags", "agent_compatible"]:
        if list_key in meta and not (
            isinstance(meta[list_key], list) and all(isinstance(x, str) for x in meta[list_key])
        ):
            fail(f"{skill_dir}: {list_key} must be an array of strings")
            errors += 1

    if "maturity" in meta and not (
        isinstance(meta.get("maturity"), str) and meta.get("maturity", "").strip()
    ):
    ...
    for field, entries in normalized_dependencies.items():
        for entry in entries:
            ...
            if registry and registry not in KNOWN_REGISTRIES:
                fail(
                    f"{skill_dir}: {field} entry for {entry.get('name')} references unknown registry {registry!r}"
                )
                errors += 1

    for nullable_key in ["derived_from", "replaces"]:
        if (
            nullable_key in meta
            and meta[nullable_key] is not None
            and not isinstance(meta[nullable_key], str)
        ):
    ...
    if requires and not isinstance(requires, dict):
        ...
        for key in ["tools", "bins", "env"]:
            if key in requires and not (
                isinstance(requires[key], list) and all(isinstance(x, str) for x in requires[key])
            ):
    ...
    if entrypoints and not isinstance(entrypoints, dict):
        ...
        skill_md_rel = (
            entrypoints.get("skill_md", "SKILL.md") if isinstance(entrypoints, dict) else "SKILL.md"
        )
    ...
    if tests and not isinstance(tests, dict):
        ...
        smoke_rel = (
            tests.get("smoke", "tests/smoke.md") if isinstance(tests, dict) else "tests/smoke.md"
        )
    ...
    if not isinstance(distribution, dict):
        ...
        if not isinstance(distribution.get("installable"), bool):
            ...
            errors += 1
        if (
            not isinstance(distribution.get("channel"), str)
            or not distribution.get("channel", "").strip()
        ):
        ...
        for arg in args:
            ...
            if p.is_dir() and ((p / "_meta.json").exists() or is_canonical_skill_dir(p)):
                ...
                for child in sorted(
                    x.resolve()
                    for x in p.iterdir()
                    if x.is_dir() and ((x / "_meta.json").exists() or is_canonical_skill_dir(x))
                ):
                    dirs.append(child)
            else:
...
def build_namespace_policy_trace(skill_dir: Path, namespace_policy, effective_sources):
    ...
    if report.get("transfer_required"):
        reasons.append(
            "namespace transfer review was required because another claim for the same skill name exists"
        )
    if competing_claims:
    ...
    return {
        ...
        **build_policy_trace(
            ...
            summary="namespace policy accepted the skill identity claim"
            if report.get("authorized")
            else "namespace policy rejected the skill identity claim",
            effective_sources=effective_sources,
            applied_rules=[
                {
                    "rule": "publisher claims must exist in namespace policy",
                    "value": identity.get("publisher") or "unqualified",
                },
                {
                    "rule": "owners and maintainers must be authorized for the publisher",
                    "value": (identity.get("qualified_name") or identity.get("name")),
                },
                {
                    "rule": "conflicting publisher claims require an authorized transfer",
                    "value": report.get("transfer_required", False),
                },
            ],
            blocking_rules=[
                {"rule": message, "message": message} for message in report.get("errors", [])
            ],
            reasons=reasons,
            next_actions=[
            ...
            else ["namespace policy accepted the current identity claim"],
        ),
    }
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Validate registry skill directories and generated catalogs"
    )
    parser.add_argument(
        "paths", nargs="*", help="Skill directories or parent directories to validate"
    )
    parser.add_argument(
        "--json", action="store_true", help="Print machine-readable validation output"
    )
    parser.add_argument(
        "--debug-policy", action="store_true", help="Print human-readable policy traces"
    )
...
def main():
    ...
    if not dirs:
        print(
            "No skill directories found." if len(args.paths) == 0 else "Nothing to validate.",
            file=sys.stderr,
        )
        return 1
    captured_errors = []
        ...
        for d in dirs:
            ...
            if _is_registry_skill_dir(d):
                policy_traces.append(
                    build_namespace_policy_trace(d, namespace_policy, namespace_policy_sources)
                )
