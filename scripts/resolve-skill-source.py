...
def scan_registry(
    reg, *, reg_root=None, reg_info=None, default_source_type="working-tree", snapshot_fields=None
):
            ...
            append_registry_item(
                ...
                items,
                {
                    ...
                    **snapshot_fields,
                    ...
                    if distribution
                    else str(d.relative_to(reg_root)),
                    "dir_name": d.name,
                    "name": meta.get("name"),
        ...
        append_registry_item(
            ...
            items,
            {
                ...
                **snapshot_fields,
                ...
                "registry_tag": distribution.get("source_snapshot_tag")
                or reg_info.get("registry_tag"),
                "registry_ref": distribution.get("source_snapshot_ref")
                or reg_info.get("registry_ref"),
            },
        )
    return items
...
def load_candidates(registry=None, snapshot=None):
    ...
    if snapshot:
        ...
        source_registry = (
            metadata.get("source_registry")
            if isinstance(metadata.get("source_registry"), dict)
            else {}
        )
        ...
        if snapshot_root is None or not Path(snapshot_root).exists():
            raise SystemExit(
                f"snapshot '{summary.get('snapshot_id') or snapshot}' for registry '{registry}' is missing its registry tree"
            )
...
def archived_snapshot_sort_key(item):
    ...
    ts_num = (
        int(ts.replace("T", "").replace("Z", "").replace("-", "").replace(":", "")) if ts else 0
    )
    return (
        -int(item.get("registry_priority", 0)),
        0 if item.get("source_type") == "distribution-manifest" else 1,
...
def main():
    ...
    if args.version:
        ...
        archived_snapshots = [
            x for x in exact if x["stage"] == "archived" and x.get("snapshot_of") in snapshot_refs
        ]
    ...
    if resolved is None:
        if args.registry:
            blocked_registry = next(
                (item for item in blocked if item.get("registry") == args.registry), None
            )
        ...
        raise SystemExit(
            f"No matching skill source found for {args.name}{'@' + args.version if args.version else ''}{suffix}."
        )
