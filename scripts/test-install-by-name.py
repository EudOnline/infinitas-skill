...
def main():
    ...
    try:
        ...
        if entry.get("installed_version") != FIXTURE_VERSION:
            fail(
                f"expected installed_version {FIXTURE_VERSION!r}, got {entry.get('installed_version')!r}"
            )
        ...
        if entry.get("install_target") != str(target_dir.resolve()):
            fail(
                f"expected install_target {str(target_dir.resolve())!r}, got {entry.get('install_target')!r}"
            )
        ...
        if payload.get("requires_confirmation") is not True:
            fail(
                f"expected requires_confirmation true, got {payload.get('requires_confirmation')!r}"
            )
        ...
        if incompatible_payload.get("error_code") != "incompatible-target-agent":
            fail(
                f"expected incompatible-target-agent, got {incompatible_payload.get('error_code')!r}"
            )
        if not incompatible_payload.get("suggested_action"):
            fail(
                f"expected suggested_action on incompatible install failure, got {incompatible_payload!r}"
            )
        explanation = incompatible_payload.get("explanation") or {}
        if not explanation.get("policy_reasons"):
            fail(
                f"expected explanation policy_reasons on incompatible install failure, got {incompatible_payload!r}"
            )
    finally:
