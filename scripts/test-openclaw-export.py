...
def test_confirm_mode_returns_release_export_plan():
    ...
    try:
        ...
        if payload.get("suggested_publish_command") != [
            "clawhub",
            "publish",
            str((out_dir / FIXTURE_NAME).resolve()),
        ]:
            fail(f"unexpected publish suggestion: {payload.get('suggested_publish_command')!r}")
        if payload.get("public_ready") is not False:
            fail(
                f"expected confirm-mode public_ready=false without MIT-0 metadata, got {payload.get('public_ready')!r}"
            )
        validation_errors = payload.get("validation_errors") or []
        if not any("MIT-0" in item for item in validation_errors):
            fail(
                f"expected confirm-mode validation errors to mention MIT-0, got {validation_errors!r}"
            )
    finally:
...
def test_auto_mode_materializes_release_bundle_as_openclaw_folder():
    ...
    try:
        ...
        if payload.get("resolved_version") != FIXTURE_VERSION:
            fail(
                f"expected resolved_version {FIXTURE_VERSION!r}, got {payload.get('resolved_version')!r}"
            )
        if payload.get("suggested_publish_command") != [
            "clawhub",
            "publish",
        ...
        if payload.get("public_ready") is not False:
            fail(
                f"expected auto-mode public_ready=false without MIT-0 metadata, got {payload.get('public_ready')!r}"
            )
        validation_errors = payload.get("validation_errors") or []
        if not any("MIT-0" in item for item in validation_errors):
            fail(
                f"expected auto-mode validation errors to mention MIT-0, got {validation_errors!r}"
            )

        run(
            [
                sys.executable,
                str(repo / "scripts" / "check-openclaw-compat.py"),
                "--skill-dir",
                str(export_dir),
            ],
            cwd=repo,
