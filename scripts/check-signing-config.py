...
def main():
    ...
    if not is_nonempty_string(attestation.get("signature_ext")) or not attestation.get(
        "signature_ext", ""
    ).startswith("."):
    ...
    top_allowed_path = (
        validate_allowed_signers(top_allowed, "allowed_signers", errors, warnings)
        if top_allowed
        else None
    )
    git_tag_allowed_path = (
        validate_allowed_signers(
            git_tag.get("allowed_signers"), "git_tag.allowed_signers", errors, warnings
        )
        if git_tag.get("allowed_signers")
        else None
    )
    attestation_allowed_path = (
        validate_allowed_signers(
            attestation.get("allowed_signers"), "attestation.allowed_signers", errors, warnings
        )
        if attestation.get("allowed_signers")
        else None
    )
    ...
    if (
        top_allowed_path
        and attestation_allowed_path
        and top_allowed_path != attestation_allowed_path
    ):
        errors.append(
            "attestation.allowed_signers must resolve to the same path as allowed_signers"
        )
    if (
        is_nonempty_string(attestation.get("namespace"))
        and is_nonempty_string(top_namespace)
        and attestation.get("namespace") != top_namespace
    ):
        errors.append("attestation.namespace must match namespace")
    if (
        is_nonempty_string(attestation.get("signature_ext"))
        and is_nonempty_string(top_ext)
        and attestation.get("signature_ext") != top_ext
    ):
