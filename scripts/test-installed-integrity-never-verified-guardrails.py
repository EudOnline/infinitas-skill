...
def main():
    ...
    if "report-installed-integrity.py" not in (
        warn_decision.get("warning") or ""
    ) or "--refresh" not in (warn_decision.get("warning") or ""):
    ...
    if "report-installed-integrity.py" not in (
        fail_decision.get("warning") or ""
    ) or "--refresh" not in (fail_decision.get("warning") or ""):
    ...
    if legacy_fail_decision.get("blocking") is not True:
        fail(
            f"expected legacy fail policy to block overwrite-style mutation, got {legacy_fail_decision!r}"
        )
    if legacy_fail_decision.get("mutation_reason_code") != "never-verified-installed-integrity":
        fail(
            f"expected legacy fail policy to surface never-verified reason code, got {legacy_fail_decision!r}"
        )
    ...
    if stale_fail_decision.get("mutation_reason_code") == "never-verified-installed-integrity":
        fail(
            f"expected stale decision to remain distinct from never-verified, got {stale_fail_decision!r}"
        )
