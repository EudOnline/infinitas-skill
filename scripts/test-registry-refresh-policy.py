...
def registry_config_with_remote(
    remote_url: str, *, stale_policy="warn", interval_hours=24, max_cache_age_hours=72
):
    config = load_base_config()
    config["default_registry"] = "upstream"
    config["registries"] = [
...
def scenario_sync_writes_refresh_state_and_status():
    ...
    try:
        ...
        write_json(
            repo / "config" / "registry-sources.json", registry_config_with_remote(str(remote))
        )
        ...
        if payload.get("cache_path") != str(
            (repo / ".cache" / "registries" / "upstream").resolve()
        ):
        ...
        result = run(
            ["python3", "scripts/registry-refresh-status.py", "upstream", "--json"], cwd=repo
        )
...
def scenario_stale_warn_allows_resolution_with_warning():
    ...
    try:
        ...
        write_json(
            ...
            registry_config_with_remote(str(remote), stale_policy="warn"),
        )
        ...
        if (
            payload.get("source_snapshot_commit") not in {None, expected_commit}
            and payload.get("registry_commit") != expected_commit
        ):
        ...
        if (
            ...
            or "scripts/sync-registry-source.sh upstream" not in warning
        ):
            fail(f"expected actionable registry_freshness_warning, got {payload!r}")
        if (
            payload.get("registry_refresh_age_hours") is None
            or payload.get("registry_refresh_age_hours") <= 72
        ):
...
def scenario_stale_fail_blocks_resolution_until_resynced():
    ...
    try:
        ...
        write_json(
            repo / "config" / "registry-sources.json",
            registry_config_with_remote(str(remote), stale_policy="fail"),
        )
