...
def scaffold_canonical_skill(repo: Path):
    ...
    write_json(
        skill_dir / "skill.json",
        {
            ...
            "degrades_to": {"no_subagent_dispatch": "sequential"},
            "distribution": {
...
def main():
    ...
    try:
        ...
        if body_path.name != "instructions.md":
            fail(
                f"expected canonical instructions body path to end with instructions.md, got {body_path!r}"
            )
        ...
        if legacy_path.name != "SKILL.md":
            fail(
                f"expected legacy instructions body path to end with SKILL.md, got {legacy_path!r}"
            )
        ...
        result = run(
            [
                ...
                "skills-src/demo-skill",
            ],
            cwd=repo,
        )
        combined = result.stdout + result.stderr
        if (
            "validated 1 skill directories" not in combined
            and "validated 1 canonical skill sources" not in combined
        ):
