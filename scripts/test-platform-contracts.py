...
def write_contract(
    repo: Path, platform: str, title: str, *, date="2026-03-12", url="https://example.com/source"
):
...
def main():
    ...
    try:
        ...
        write_contract(
            stale_age_repo,
            "claude",
            REQUIRED_DOCS["claude"],
            date="2025-01-01",
            url="https://docs.example.com/claude",
        )
        write_profile_contract(
            stale_age_repo, "claude", date="2025-01-01", urls=["https://docs.example.com/claude"]
        )
        run(
        ...
        write_profile_contract(
            mismatch_repo, "codex", date="2026-03-11", urls=["https://docs.example.com/codex"]
        )
        ...
        write_profile_contract(
            invalid_date_repo, "claude", date="not-a-date", urls=["https://docs.example.com/claude"]
        )
