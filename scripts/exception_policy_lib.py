...
def _normalize_string_list(values):
    ...
    if not isinstance(values, list):
        return []
    return _unique_strings([item.strip() for item in values if isinstance(item, str) and item.strip()])
...
def validate_exception_policy(payload, team_policy):
    ...
    for index, raw_record in enumerate(raw_exceptions, start=1):
        ...
        if not skills:
            record_errors.append(f'{label}.skills must include at least one exact skill name or qualified name')
        if any('*' in item or '?' in item for item in skills):
            record_errors.append(f'{label}.skills must use exact names only; wildcards are not supported')
        ...
        if any('*' in item or '?' in item for item in rules):
            record_errors.append(f'{label}.rules must use exact rule ids only; wildcards are not supported')

        approvers = expand_exception_approvers(raw_record, team_policy)
        if raw_record.get('approved_by') is not None and not isinstance(raw_record.get('approved_by'), list):
            record_errors.append(f'{label}.approved_by must be an array when present')
        if raw_record.get('approved_by_teams') is not None and not isinstance(raw_record.get('approved_by_teams'), list):
        ...
        for missing_team in approvers.get('missing_approved_by_teams', []):
            record_errors.append(f'{label}.approved_by_teams references unknown team {missing_team!r}')
        if not approvers.get('resolved_approved_by'):
            record_errors.append(f'{label} must resolve at least one approver via approved_by or approved_by_teams')

        approved_at, approved_at_dt = _parse_timestamp(f'{label}.approved_at', raw_record.get('approved_at'), errors=record_errors)
        expires_at, expires_at_dt = _parse_timestamp(f'{label}.expires_at', raw_record.get('expires_at'), errors=record_errors)
...
def load_exception_policy(root=ROOT):
    ...
    try:
        ...
        missing = f"missing policy source for domain 'exception_policy'"
...
def match_active_exceptions(scope, skill_identity, blocking_rule_ids, root=ROOT, policy=None):
    ...
    for record in (policy.get('exceptions') or []):
        ...
        if not approved_at_dt or not expires_at_dt or approved_at_dt > now or expires_at_dt <= now:
            continue
        matched_rules = [rule for rule in record.get('rules', []) if rule in requested_rules]
        ...
        usage.append(
            {
                ...
                'skills': copy.deepcopy(record.get('skills', [])),
                'rules': copy.deepcopy(record.get('rules', [])),
                'matched_rules': matched_rules,
                'approved_by': copy.deepcopy(record.get('approved_by', [])),
                'approved_by_teams': copy.deepcopy(record.get('approved_by_teams', [])),
                'resolved_approved_by': copy.deepcopy(record.get('resolved_approved_by', [])),
