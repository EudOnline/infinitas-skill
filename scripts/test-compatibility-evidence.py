...
def main():
    ...
    try:
        ...
        if verified.get("codex", {}).get("freshness_reason") != "contract-newer-than-evidence":
            fail(
                f"expected codex freshness_reason contract-newer-than-evidence, got {verified.get('codex')!r}"
            )
        if (
            verified.get("codex", {}).get("contract_last_verified")
            != codex_contract_day.isoformat()
        ):
        ...
        if verified.get("openclaw", {}).get("freshness_reason") != "not-applicable":
            fail(
                f"expected openclaw freshness_reason not-applicable, got {verified.get('openclaw')!r}"
            )
        if (
            verified.get("openclaw", {}).get("contract_last_verified")
            != openclaw_contract_day.isoformat()
        ):
            fail(
                f"expected openclaw contract_last_verified 2026-03-12, got {verified.get('openclaw')!r}"
            )
        ...
        if verified.get("openclaw", {}).get("fresh_until") != expected_openclaw_fresh_until:
            fail(
                f"expected openclaw fresh_until 2026-04-11T12:05:00Z, got {verified.get('openclaw')!r}"
            )
        ...
        if verified.get("claude", {}).get("freshness_reason") != "missing-evidence":
            fail(
                f"expected claude freshness_reason missing-evidence, got {verified.get('claude')!r}"
            )
        if (
            verified.get("claude", {}).get("contract_last_verified")
            != claude_contract_day.isoformat()
        ):
            fail(
                f"expected claude contract_last_verified 2026-03-12, got {verified.get('claude')!r}"
            )
    finally:
