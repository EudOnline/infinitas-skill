...
def scaffold_skill(repo: Path, stage: str, dir_name: str, *, version: str, publisher='lvxiaoer', snapshot_of=None, snapshot_created_at=None, with_schema=True):
...
def main():
    ...
    try:
        ...
        result = run([sys.executable, str(repo / 'scripts' / 'resolve-skill-source.py'), 'demo-skill', '--json'], cwd=repo)
        payload = json.loads(result.stdout)
        if payload.get('qualified_name') != 'lvxiaoer/demo-skill':
            fail(f'bare skill name should still resolve with qualified identity present\n{result.stdout}')

        result = run([sys.executable, str(repo / 'scripts' / 'resolve-skill-source.py'), 'demo-skill', '--version', '1.2.3', '--json'], cwd=repo)
        payload = json.loads(result.stdout)
        if payload.get('stage') != 'archived' or payload.get('resolution_reason') != 'archived-exact-snapshot':
