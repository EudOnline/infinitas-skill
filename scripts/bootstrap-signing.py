...
def parse_args():
    parser = argparse.ArgumentParser(description='Bootstrap SSH signing and repository signer policy')
    subparsers = parser.add_subparsers(dest='command', required=True)

    init_key = subparsers.add_parser('init-key', help='Generate a new SSH signing key pair')
    init_key.add_argument('--identity', required=True, help='Signer identity to store as the SSH key comment')
    ...
    add_signer = subparsers.add_parser('add-allowed-signer', help='Add or update a trusted signer entry')
    add_signer.add_argument('--identity', required=True, help='Signer identity stored in config/allowed_signers')
    ...
    configure = subparsers.add_parser('configure-git', help='Configure git to use SSH signing with a key path')
    configure.add_argument('--key', required=True, help='Private SSH signing key path')
    configure.add_argument('--scope', choices=['local', 'global'], default='local', help='Git config scope')

    authorize = subparsers.add_parser('authorize-publisher', help='Authorize signer or releaser identities for a publisher')
    authorize.add_argument('--publisher', required=True, help='Publisher slug in policy/namespace-policy.json')
    authorize.add_argument('--signer', action='append', default=[], help='Signer identity to add to authorized_signers')
    authorize.add_argument('--releaser', action='append', default=[], help='Releaser identity to add to authorized_releasers')
...
def run_init_key(args):
    ...
    print('next: python3 scripts/bootstrap-signing.py add-allowed-signer --identity ' f'{args.identity} --key {output}')


def run_add_allowed_signer(args):
    ...
    print('next: git add config/allowed_signers && git commit -m "chore: trust release signer" && git push')


def run_configure_git(args):
...
def run_authorize_publisher(args):
    if not args.signer and not args.releaser:
        raise SigningBootstrapError('authorize-publisher requires at least one --signer or --releaser')
    ...
    print('next: git add policy/namespace-policy.json && git commit -m "chore: authorize release identities" && git push')
