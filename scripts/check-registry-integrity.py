...
def main():
    ...
    active_names = {
        meta.get("name"): (skill_dir, meta, entry) for skill_dir, meta, entry in active_installable
    }
    ...
    if errors:
        print(
            f"Integrity check failed with {errors} error(s) and {warnings} warning(s).",
            file=sys.stderr,
        )
        return 1
    print(
        f"OK: registry integrity checked across {len(items)} skill directories ({warnings} warning(s))"
    )
    return 0
