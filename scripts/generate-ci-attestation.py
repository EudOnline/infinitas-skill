...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Generate a CI-native release attestation payload for a skill"
    )
    parser.add_argument("skill", help="Skill name or path")
    parser.add_argument(
        "--output-name", help="Final attestation filename for CI-side sidecar naming"
    )
    parser.add_argument("--releaser", help="Release operator identity override")
    parser.add_argument(
        "--distribution-manifest-path", help="Repository-relative distribution manifest path"
    )
    parser.add_argument(
        "--distribution-bundle-path", help="Repository-relative distribution bundle path"
    )
    parser.add_argument(
        "--distribution-bundle-sha256", help="SHA-256 digest of the distribution bundle"
    )
    parser.add_argument(
        "--distribution-bundle-size", type=int, help="Size in bytes of the distribution bundle"
    )
    parser.add_argument(
        "--distribution-bundle-root-dir",
        help="Top-level extracted directory name inside the bundle",
    )
    parser.add_argument(
        "--distribution-bundle-file-count", type=int, help="Number of files archived in the bundle"
    )
...
def main():
    ...
    output_name = (
        args.output_name
        or f"{context['meta'].get('name')}-{context['meta'].get('version')}.ci.json"
    )
