...
def _persist_distribution_cache(target_dir: Path, source_info):
    ...
    if (
        source_info.get("source_type") != "distribution-manifest"
        or source_info.get("registry_kind") != "http"
    ):
        return None
    ...
    for label, rel in refs.items():
        if not isinstance(rel, str) or not rel:
            raise SystemExit(
                f"FAIL: hosted source is missing distribution {label} reference for cache persistence"
            )
    ...
    cache_root = (
        target_dir / ".infinitas-skill-distributions" / skill_id / version / digest[:16]
    ).resolve()
    ...
    for rel in refs.values():
        ...
        if ref.is_absolute():
            raise SystemExit(
                f"FAIL: hosted distribution cache reference must be relative, got: {rel}"
            )
...
if len(sys.argv) not in {7, 8}:
    print(
        "usage: scripts/update-install-manifest.py <target-dir> <source-dir> <dest-dir> <action> <locked-version> <resolved-source-json> [resolution-plan-json]",
        file=sys.stderr,
    )
    raise SystemExit(1)
...
previous_events = (
    normalize_integrity_events(previous.get("integrity_events"))
...
manifest_entry = {
    ...
    "source_stage": source_info.get("source_stage")
    or source_info.get("stage")
    or source_dir.parent.name,
    ...
    "source_attestation_signature_sha256": source_info.get(
        "distribution_attestation_signature_sha256"
    ),
...
if snapshot_path.exists() or any(
    archived_by_name.get(skill_name) for skill_name in archived_by_name
):
    write_installed_integrity_snapshot(
        target_dir,
        manifest,
