...
def main():
    ...
    try:
        ...
        if payload["install_policy"]["mode"] != "immutable-only":
            fail(
                f"expected immutable-only install policy, got {payload['install_policy']['mode']!r}"
            )
        ...
        if entry.get("capabilities") != ["fixture-testing", "search"]:
            fail(
                f"expected capabilities ['fixture-testing', 'search'], got {entry.get('capabilities')!r}"
            )
        ...
        if entry.get("runtime_assumptions") != ["A local repo checkout is available"]:
            fail(
                f"expected canonical runtime_assumptions, got {entry.get('runtime_assumptions')!r}"
            )
        ...
        for platform, payload in (entry.get("verified_support") or {}).items():
            ...
            if (
                not isinstance(payload.get("freshness_state"), str)
                or not payload.get("freshness_state").strip()
            ):
                fail(
                    f"expected ai-index verified_support {platform} freshness_state, got {payload!r}"
                )
        ...
        if not isinstance(version_entry.get("attestation_formats"), list) or not version_entry.get(
            "attestation_formats"
        ):
            fail(
                f"expected attestation_formats for version entry, got {version_entry.get('attestation_formats')!r}"
            )
        if (
            not isinstance(version_entry.get("trust_state"), str)
            or not version_entry.get("trust_state").strip()
        ):
            fail(
                f"expected version trust_state to be non-empty, got {version_entry.get('trust_state')!r}"
            )

        interop = (entry.get("interop") or {}).get("openclaw") or {}
        if interop.get("import_supported") is not True:
            fail(
                f"expected interop.openclaw.import_supported=true, got {interop.get('import_supported')!r}"
            )
        if interop.get("export_supported") is not True:
            fail(
                f"expected interop.openclaw.export_supported=true, got {interop.get('export_supported')!r}"
            )
        ...
        target = next(
            (item for item in original_ai_index["skills"] if item.get("name") == FIXTURE_NAME), None
        )
        if target is None:
        ...
        result = run(
            [sys.executable, str(repo / "scripts" / "validate-registry.py")], cwd=repo, expect=1
        )
        ...
        rebuilt_entry = next(
            (item for item in rebuilt_payload["skills"] if item.get("name") == FIXTURE_NAME), None
        )
        rebuilt_entry["versions"][FIXTURE_VERSION]["distribution_manifest_path"] = (
            "/tmp/manifest.json"
        )
        ...
        skill_md_path.write_text(
            original_skill_md.replace(f"name: {FIXTURE_NAME}", "name: wrong-name", 1),
            encoding="utf-8",
        )
        result = run(
            [sys.executable, str(repo / "scripts" / "validate-registry.py")], cwd=repo, expect=1
        ...
        skill_md_path.write_text(
            original_skill_md.replace(
                "description: Fixture skill for ai-index tests.", "description:", 1
            ),
            encoding="utf-8",
        )
        result = run(
