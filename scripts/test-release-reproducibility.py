...
def scenario_release_outputs_include_reproducibility_metadata():
    ...
    try:
        ...
        provenance_file_manifest = expect_file_manifest(
            provenance_distribution, "provenance.distribution"
        )
        ...
        if "SKILL.md" not in manifest_paths or "_meta.json" not in manifest_paths:
            fail(
                f"expected file manifest to include SKILL.md and _meta.json, got {sorted(manifest_paths)!r}"
            )
        if provenance_build.get("archive_format") != manifest_build.get("archive_format"):
            fail(
                f"expected provenance and manifest build metadata to agree, got {provenance_build!r} vs {manifest_build!r}"
            )
    finally:
...
def scenario_verification_and_release_state_surface_reproducibility_metadata():
    ...
    try:
        ...
        if distribution.get("file_manifest_count", 0) < 1:
            fail(
                f"expected verify-attestation JSON to expose file_manifest_count, got {verified_payload!r}"
            )
        build = distribution.get("build") or {}
        if build.get("archive_format") != "tar.gz":
            fail(
                f"expected verify-attestation JSON to expose build metadata, got {verified_payload!r}"
            )
        ...
        if reproducibility.get("file_manifest_count", 0) < 1:
            fail(
                f"expected check-release-state JSON to expose reproducibility summary, got {release_payload!r}"
            )
        if reproducibility.get("archive_format") != "tar.gz":
            fail(
                f"expected check-release-state JSON to expose archive_format, got {release_payload!r}"
            )
        ...
        catalog_item = next(
            (item for item in catalog.get("skills") or [] if item.get("name") == FIXTURE_NAME), None
        )
        if not catalog_item:
        ...
        if verified_distribution.get("file_manifest_count", 0) < 1:
            fail(
                f"expected catalog verified_distribution to expose file_manifest_count, got {catalog_item!r}"
            )
        if verified_distribution.get("build_archive_format") != "tar.gz":
            fail(
                f"expected catalog verified_distribution to expose build_archive_format, got {catalog_item!r}"
            )
    finally:
        shutil.rmtree(tmpdir)
...
def scenario_distribution_verification_rejects_file_manifest_mismatch():
    ...
    try:
        ...
        if not file_manifest:
            fail(
                f"expected manifest to include file_manifest before mismatch check, got {manifest!r}"
            )
