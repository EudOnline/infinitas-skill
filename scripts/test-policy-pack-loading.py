...
def scenario_declared_pack_order_and_local_overrides():
    ...
    try:
        ...
        if sources[-1].get("path") != "config/signing.json":
            fail(
                f"expected local override path 'config/signing.json', got {sources[-1].get('path')!r}"
            )
        ...
        if policy.get("release_trust_mode") != "ci":
            fail(
                f"expected repo-local signing override to win, got {policy.get('release_trust_mode')!r}"
            )
        ...
        required_groups = (
            ((reviews.get("quorum") or {}).get("stage_overrides") or {}).get("active") or {}
        ).get("required_groups")
        if required_groups != ["maintainers", "security"]:
            fail(
                f"expected repository-local stage override to replace required_groups, got {required_groups!r}"
            )
        ...
        if sorted(publishers) != ["baseline", "dual"]:
            fail(
                f"expected namespace policy publishers from both packs, got {sorted(publishers)!r}"
            )
        ...
        if federation.get("allowed_publishers") != ["partner"]:
            fail(
                f"expected registry_sources federation.allowed_publishers ['partner'], got {federation!r}"
            )
        if federation.get("publisher_map") != {"partner": "partner-labs"}:
            fail(
                f"expected registry_sources federation.publisher_map to survive pack merge, got {federation!r}"
            )
        if federation.get("require_immutable_artifacts") is not True:
            fail(
                f"expected registry_sources federation.require_immutable_artifacts true, got {federation!r}"
            )
