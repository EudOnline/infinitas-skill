...
def _confidence_view(*, factors: dict, rank: int, score_gap_from_top: int, score_gap_to_runner_up: int | None = None) -> dict:
    match_strength = factors.get('match_strength') if isinstance(factors.get('match_strength'), int) else 0
...
def _comparison_summary(winner: dict, runner_up: dict, score_gap: int) -> str:
    ...
    return (
        f"{winner.get('qualified_name')} leads {runner_up.get('qualified_name')} by {score_gap} score points; "
        ...
        for rank, entry in enumerate(scored, start=1):
            ...
            if runner_up and rank == 1:
                ...
                result['comparative_signals']['quality_gap_from_runner_up'] = _non_negative_gap(
                    (factors.get('quality') if isinstance(factors.get('quality'), int) else 0),
                    (runner_up_factors.get('quality') if isinstance(runner_up_factors.get('quality'), int) else 0),
                )
                result['comparative_signals']['verification_freshness_gap_from_runner_up'] = _non_negative_gap(
                    (
                        factors.get('verification_freshness_score')
                        if isinstance(factors.get('verification_freshness_score'), int)
                        else 0
                ...
                result['comparative_signals']['compatibility_gap_from_runner_up'] = _compatibility_gap(
                    runner_up_factors,
                    factors,
                )
