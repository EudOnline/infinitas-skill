...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Export skills through the real platform adapters, run platform checks, and write compatibility evidence."
    )
    ...
    parser.add_argument(
        ...
        dest="platforms",
        default=[],
        help="Platform to verify (repeatable): claude, codex, openclaw",
    )
    parser.add_argument(
        "--version", help="Explicit version to verify; defaults to the current skill version"
    )
    parser.add_argument(
        "--build-catalog", action="store_true", help="Rebuild catalog views after writing evidence"
    )
...
def ensure_version(meta: dict, requested_version: str | None) -> str:
    ...
    if requested_version and requested_version != version:
        raise VerifiedSupportError(
            f"--version {requested_version!r} does not match skill version {version!r}"
        )
    return version
...
def verify_codex(
    root: Path, skill_dir: Path, skill_name: str, qualified_name: str | None, version: str
):
    with tempfile.TemporaryDirectory(prefix=f"infinitas-codex-verify-{skill_name}-") as tmpdir:
        out_dir = Path(tmpdir) / "codex"
        run(
            [
                str(root / "scripts" / "export-codex-skill.sh"),
                "--skill-dir",
                str(skill_dir),
                "--out",
                str(out_dir),
        ...
        run(
            [
                sys.executable,
                str(root / "scripts" / "check-codex-compat.py"),
                "--skill-dir",
                str(out_dir),
            ],
            cwd=root,
...
def verify_claude(
    ...
    with tempfile.TemporaryDirectory(prefix=f"infinitas-claude-verify-{skill_name}-") as tmpdir:
        ...
        run(
            [
                str(root / "scripts" / "export-claude-skill.sh"),
                "--skill-dir",
                str(skill_dir),
                "--out",
                str(out_dir),
        ...
        run(
            [
                sys.executable,
                str(root / "scripts" / "check-claude-compat.py"),
                "--skill-dir",
                str(out_dir),
            ],
            cwd=root,
...
def verify_openclaw(root: Path, skill_name: str, qualified_name: str | None, version: str):
    ...
    with tempfile.TemporaryDirectory(prefix=f"infinitas-openclaw-verify-{skill_name}-") as tmpdir:
        ...
        run(
            [
                sys.executable,
                str(root / "scripts" / "check-openclaw-compat.py"),
                "--skill-dir",
                str(export_dir),
            ],
            cwd=root,
