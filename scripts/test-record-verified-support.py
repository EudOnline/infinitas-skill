...
from infinitas_skill.testing.env import (
...
def make_env(extra=None):
    return build_regression_test_env(
        ...
        extra=extra,
        env=os.environ.copy(),
        skip_test_flags=DEFAULT_SKIP_TEST_FLAGS + ("INFINITAS_SKIP_RECORD_VERIFIED_SUPPORT_TESTS",),
    )
...
def rewrite_namespace_policy(repo: Path):
    ...
    authorized_signers = (
        lvxiaoer.get("authorized_signers")
        if isinstance(lvxiaoer.get("authorized_signers"), list)
        else []
    )
    authorized_releasers = (
        lvxiaoer.get("authorized_releasers")
        if isinstance(lvxiaoer.get("authorized_releasers"), list)
        else []
    )

    lvxiaoer["authorized_signers"] = list(dict.fromkeys([*authorized_signers, "release-test"]))
    lvxiaoer["authorized_releasers"] = list(
        dict.fromkeys([*authorized_releasers, "Verified Fixture"])
    )
    ...
    try:
        ...
        for platform in ["codex", "claude", "openclaw"]:
            ...
            if payload.get("state") != "adapted":
                fail(
                    f"expected adapted evidence state for {platform}, got {payload.get('state')!r}"
                )

        compatibility = json.loads(
            (repo / "catalog" / "compatibility.json").read_text(encoding="utf-8")
        )
        entry = next(
            (item for item in compatibility.get("skills", []) if item.get("name") == FIXTURE_NAME),
            None,
        )
        if entry is None:
        ...
        for platform in ["codex", "claude", "openclaw"]:
            profile = json.loads(
                (repo / "profiles" / f"{platform}.json").read_text(encoding="utf-8")
            )
            contract_dates[platform] = (profile.get("contract") or {}).get("last_verified")
        for platform in ["codex", "claude", "openclaw"]:
            if verified.get(platform, {}).get("state") != "adapted":
                fail(
                    f"expected verified_support {platform}=adapted, got {verified.get(platform)!r}"
                )
            if verified.get(platform, {}).get("freshness_state") != "fresh":
                fail(
                    f"expected verified_support {platform} freshness_state=fresh, got {verified.get(platform)!r}"
                )
            if verified.get(platform, {}).get("freshness_reason") != "not-applicable":
                fail(
                    f"expected verified_support {platform} freshness_reason=not-applicable, got {verified.get(platform)!r}"
                )
            ...
            if not isinstance(
                verified.get(platform, {}).get("fresh_until"), str
            ) or not verified.get(platform, {}).get("fresh_until"):
                fail(
                    f"expected verified_support {platform} fresh_until string, got {verified.get(platform)!r}"
                )

        distributions = json.loads(
            (repo / "catalog" / "distributions.json").read_text(encoding="utf-8")
        )
