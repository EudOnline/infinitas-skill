...
def main():
    ...
    try:
        ...
        if payload.get("resolved_version") != FIXTURE_VERSION:
            fail(
                f"expected resolved version {FIXTURE_VERSION!r}, got {payload.get('resolved_version')!r}"
            )
        ...
        if external_payload.get("qualified_name") != f"partner/{EXTERNAL_SKILL_NAME}":
            fail(
                f"expected external qualified_name partner/{EXTERNAL_SKILL_NAME!s}, got {external_payload.get('qualified_name')!r}"
            )
        if external_payload.get("registry_name") != EXTERNAL_REGISTRY_NAME:
            fail(
                f"expected registry_name {EXTERNAL_REGISTRY_NAME!r}, got {external_payload.get('registry_name')!r}"
            )
        if external_payload.get("resolved_version") != EXTERNAL_SKILL_VERSION:
            fail(
                f"expected resolved version {EXTERNAL_SKILL_VERSION!r}, got {external_payload.get('resolved_version')!r}"
            )
        if external_payload.get("state") != "planned":
            fail(
                f"expected planned state for external confirm mode, got {external_payload.get('state')!r}"
            )
        ...
        if missing_manifest_payload.get("error_code") != "missing-distribution-file":
            fail(
                f"expected missing-distribution-file for missing manifest, got {missing_manifest_payload!r}"
            )
        if "manifest_path" not in (missing_manifest_payload.get("message") or ""):
            fail(
                f"expected missing manifest message to mention manifest_path, got {missing_manifest_payload!r}"
            )
        explanation = missing_manifest_payload.get("explanation") or {}
        if not explanation.get("policy_reasons"):
            fail(
                f"expected explanation on missing manifest failure, got {missing_manifest_payload!r}"
            )
        ...
        if missing_bundle_payload.get("error_code") != "missing-distribution-file":
            fail(
                f"expected missing-distribution-file for missing bundle, got {missing_bundle_payload!r}"
            )
        if "bundle_path" not in (missing_bundle_payload.get("message") or ""):
            fail(
                f"expected missing bundle message to mention bundle_path, got {missing_bundle_payload!r}"
            )
        ...
        if missing_attestation_payload.get("error_code") != "missing-distribution-file":
            fail(
                f"expected missing-distribution-file for missing attestation, got {missing_attestation_payload!r}"
            )
        if "attestation_path" not in (missing_attestation_payload.get("message") or ""):
            fail(
                f"expected missing attestation message to mention attestation_path, got {missing_attestation_payload!r}"
            )
        explanation = missing_attestation_payload.get("explanation") or {}
        if not explanation.get("selection_reason"):
            fail(
                f"expected explanation on missing attestation failure, got {missing_attestation_payload!r}"
            )
