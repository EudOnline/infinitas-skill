...
def assert_inventory_export(repo: Path):
    ...
    if payload.get("default_registry") != "self":
        fail(
            f"expected inventory export default_registry 'self', got {payload.get('default_registry')!r}"
        )
    ...
    skill = next(
        (
            item
            for item in skills
            if item.get("qualified_name") == "lvxiaoer/operate-infinitas-skill"
        ),
        None,
    )
    if skill is None:
    ...
    if skill.get("release_installed_integrity_reason") is not None:
        fail(
            f"expected inventory export release_installed_integrity_reason to be omitted, got {skill!r}"
        )
...
def assert_integrity_capability_surfaces(repo: Path):
    ...
    operate = _distribution_by_key(
        dist_entries, qualified_name="lvxiaoer/operate-infinitas-skill", version="0.1.1"
    )
    if operate is None:
    ...
    if (
        not isinstance(operate.get("file_manifest_count"), int)
        or operate.get("file_manifest_count") < 1
    ):
        fail(f"expected operate file_manifest_count to be a positive integer, got {operate!r}")

    legacy = _distribution_by_key(
        dist_entries, qualified_name="lvxiaoer/federation-registry-ops", version="0.1.0"
    )
    if legacy is None:
    ...
    if legacy.get("installed_integrity_reason") != "missing-signed-file-manifest":
        fail(
            f"expected legacy installed_integrity_reason 'missing-signed-file-manifest', got {legacy!r}"
        )
    ...
    if verified_distribution.get("installed_integrity_capability") != "supported":
        fail(
            f"expected catalog verified_distribution installed_integrity_capability 'supported', got {operate_skill!r}"
        )
    if verified_distribution.get("installed_integrity_reason") is not None:
        fail(
            f"expected catalog verified_distribution installed_integrity_reason to be omitted, got {operate_skill!r}"
        )

    discovery_guidance = (repo / "docs" / "reference" / "distribution-manifests.md").read_text(
        encoding="utf-8"
    )
    if "installed_integrity_capability" not in discovery_guidance:
        fail(
            "expected docs/reference/distribution-manifests.md to mention 'installed_integrity_capability'"
        )
...
def assert_audit_export(repo: Path):
    ...
    for forbidden in ["integrity", "installed_integrity", "installed_state", "installed_runtime"]:
        if forbidden in release:
            fail(
                f"expected audit-export release entry not to include {forbidden!r}, got {release!r}"
            )
