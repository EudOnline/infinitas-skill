...
def display_name(payload):
    if not isinstance(payload, dict):
        return None
    return payload.get('qualified_name') or derive_qualified_name(payload.get('name'), payload.get('publisher')) or payload.get('name')
...
def normalize_skill_identity(meta):
    ...
    publisher = raw_publisher.strip() if isinstance(raw_publisher, str) and raw_publisher.strip() else None
    ...
    qualified_name = raw_qualified.strip() if isinstance(raw_qualified, str) and raw_qualified.strip() else None
...
def validate_identity_metadata(meta):
    ...
    if owners_value is not None:
        if not isinstance(owners_value, list) or not all(isinstance(item, str) and item.strip() for item in owners_value):
    ...
    if identity.get('owner') and identity.get('owners') and identity['owner'] not in identity['owners']:
...
def load_namespace_policy(root=ROOT):
    ...
    for index, raw_entry in enumerate(raw_transfers, start=1):
        ...
        if unknown:
            errors.append(f'namespace-policy transfers[{index}] has unsupported keys: {", ".join(unknown)}')
        ...
        if not isinstance(name, str) or not SKILL_NAME_RE.match(name):
            errors.append(f'namespace-policy transfers[{index}].name must be a lowercase skill slug')
            continue
        if from_publisher is not None and (not isinstance(from_publisher, str) or not PUBLISHER_RE.match(from_publisher)):
            errors.append(f'namespace-policy transfers[{index}].from must be null or a publisher slug')
            continue
        if to_publisher is not None and (not isinstance(to_publisher, str) or not PUBLISHER_RE.match(to_publisher)):
            errors.append(f'namespace-policy transfers[{index}].to must be null or a publisher slug')
            continue
        if not approved_by:
            errors.append(f'namespace-policy transfers[{index}].approved_by must include at least one actor')
            continue
        if note is not None and (not isinstance(note, str) or not note.strip()):
            errors.append(f'namespace-policy transfers[{index}].note must be a non-empty string when present')
            continue
        transfers.append({
            'name': name,
            'from': from_publisher,
...
def iter_registry_skill_dirs(root=ROOT):
    ...
    for stage in ['incubating', 'active', 'archived']:
        ...
        if not stage_dir.exists():
            continue
        for child in sorted(path for path in stage_dir.iterdir() if path.is_dir() and (path / '_meta.json').exists()):
            yield child.resolve()
...
def namespace_policy_report(skill_dir, root=ROOT, policy=None):
    ...
    warnings = []
    publisher_entry = policy['publishers'].get(identity.get('publisher')) if identity.get('publisher') else None
    ...
    if identity.get('publisher'):
        if publisher_entry is None:
            ...
            if missing_maintainers:
                errors.append(
                    f'{skill_dir}: maintainers {", ".join(missing_maintainers)} are not authorized for publisher {identity["publisher"]!r}'
                )
    elif not policy['compatibility'].get('allow_unqualified_names', True):
        errors.append(f'{skill_dir}: unqualified legacy skill names are disabled by policy/namespace-policy.json')
    ...
    for other_dir in iter_registry_skill_dirs(root):
        ...
        same_qualified = bool(identity.get('qualified_name') and other.get('qualified_name') == identity.get('qualified_name'))
        ...
        match = transfer_is_authorized(policy, identity.get('name'), other.get('publisher'), identity.get('publisher'))
        if not match and ('archived' in {skill_dir.parent.name, other_dir.parent.name}):
            match = transfer_is_authorized(policy, identity.get('name'), identity.get('publisher'), other.get('publisher'))
        if match:
            transfer_matches.append(match)
            continue
        transfer_authorized = False
        errors.append(
            f'{skill_dir}: namespace transfer for {identity.get("name")!r} from {other.get("publisher")!r} to {identity.get("publisher")!r} is not authorized by policy/namespace-policy.json'
        )
    ...
    if publisher_entry:
        ...
        authorized_signers = unique_strings(identity.get('owners', []) + identity.get('maintainers', []))
        authorized_releasers = unique_strings(identity.get('owners', []) + identity.get('maintainers', []))

    if not authorized_signers:
        warnings.append(f'{skill_dir}: no authorized_signers resolved for {display_name(identity) or skill_dir.name}')
