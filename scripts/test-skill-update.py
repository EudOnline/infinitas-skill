...
def update_fixture(repo: Path, version: str):
    ...
    (fixture_dir / "CHANGELOG.md").write_text(
        f"# Changelog\n\n## {version} - 2026-03-12\n- Prepared update fixture for {version}.\n",
        encoding="utf-8",
    )
    write_json(
...
def main():
    ...
    try:
        ...
        payload = json.loads(
            run_cli(
                repo, ["install", "check-update", FIXTURE_NAME, str(target_dir), "--json"]
            ).stdout
        )
        ...
        if payload.get("latest_available_version") != V2:
            fail(
                f"expected latest_available_version {V2!r}, got {payload.get('latest_available_version')!r}"
            )
        ...
        if never_verified_payload.get("freshness_state") != "never-verified":
            fail(
                f"expected never-verified freshness_state 'never-verified', got {never_verified_payload!r}"
            )
        if never_verified_payload.get("mutation_readiness") != "warning":
            fail(
                f"expected never-verified mutation_readiness 'warning', got {never_verified_payload!r}"
            )
        ...
        if (
            never_verified_payload.get("mutation_reason_code")
        ...
        if never_verified_payload.get("recovery_action") != "refresh":
            fail(
                f"expected never-verified recovery_action 'refresh', got {never_verified_payload!r}"
            )
        if never_verified_payload.get("recommended_action") != "refresh":
            fail(
                f"expected never-verified recommended_action 'refresh', got {never_verified_payload!r}"
            )
        if "report-installed-integrity.py" not in (
            never_verified_payload.get("freshness_warning") or ""
        ):
            fail(
                f"expected never-verified freshness_warning to recommend refresh, got {never_verified_payload!r}"
            )
        ...
        if stale_payload.get("next_step") != "refresh-installed-integrity":
            fail(
                f"expected stale next_step 'refresh-installed-integrity', got {stale_payload.get('next_step')!r}"
            )
        ...
        if (
            warn_upgrade_payload.get("state") != "installed"
            or warn_upgrade_payload.get("to_version") != V2
        ):
        ...
        if current.get("installed_version") != V2:
            fail(
                f"expected stale warn upgrade to install {V2!r}, got {current.get('installed_version')!r}"
            )
        ...
        if current.get("installed_version") != V1:
            fail(
                f"expected rollback after stale warn upgrade to restore {V1!r}, got {current.get('installed_version')!r}"
            )
        ...
        if blocked_payload.get("next_step") != "refresh-installed-integrity":
            fail(
                f"expected refresh next_step for stale block, got {blocked_payload.get('next_step')!r}"
            )
        ...
        if current.get("installed_version") != V1:
            fail(
                f"expected stale fail upgrade to leave installed version at {V1!r}, got {current.get('installed_version')!r}"
            )
        ...
        if (
            refreshed_upgrade.get("state") != "installed"
            or refreshed_upgrade.get("to_version") != V2
        ):
        ...
        if payload.get("update_available") is not True:
            fail(
                f"expected update_available true for drifted install, got {payload.get('update_available')!r}"
            )
        integrity = payload.get("integrity")
        if not isinstance(integrity, dict):
        ...
        if "SKILL.md" not in (integrity.get("modified_files") or []):
            fail(
                f"expected integrity.modified_files to include 'SKILL.md', got {integrity.get('modified_files')!r}"
            )
        ...
        if payload.get("mutation_reason_code") != "drifted-installed-skill":
            fail(
                f"expected drifted mutation_reason_code 'drifted-installed-skill', got {payload!r}"
            )
        ...
        if (
            "repair-installed-skill.sh" not in combined
            or "verify-installed-skill.py" not in combined
        ):
            fail(
                f"expected drifted upgrade failure to recommend verify and repair commands\n{combined}"
            )
        ...
        if current.get("installed_version") != V1:
            fail(
                f"expected repair to restore recorded version {V1!r}, got {current.get('installed_version')!r}"
            )
        ...
        if current.get("installed_version") != V2:
            fail(
                f"expected installed_version {V2!r} after upgrade, got {current.get('installed_version')!r}"
            )
        ...
        if (
            ...
            fail(
                f"expected drifted rollback failure to recommend verify and repair commands\n{combined}"
            )
