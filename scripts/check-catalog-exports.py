...
def main():
    ...
    registry_names = {
        item.get("name")
        for item in registries.get("registries") or []
        if isinstance(item, dict) and item.get("name")
    }
    inventory_registry_names = {
        item.get("name")
        for item in inventory_registries
        if isinstance(item, dict) and item.get("name")
    }
    if inventory_registry_names != registry_names:
        fail(
            f"inventory export registry names mismatch: {sorted(inventory_registry_names)!r} != {sorted(registry_names)!r}"
        )
    ...
    for key, item in catalog_skills.items():
        exported = next(
            (
                candidate
                for candidate in inventory_skills
                if isinstance(candidate, dict)
                and (
                    candidate.get("qualified_name") or candidate.get("name"),
