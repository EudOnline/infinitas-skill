...
def filter_by_agent(candidates: list, target_agent: str | None) -> list:
    ...
    return [candidate for candidate in candidates if target_agent in (candidate.get('agent_compatible') or [])]


def rank_candidates(candidates: list, *, default_registry: str, target_agent: str | None = None, query: str | None = None) -> list:
...
def resolve_skill(*, payload: dict, query: str, target_agent: str | None = None) -> dict:
    ...
    private_candidates = [item for item in all_candidates if item.get('source_registry') == default_registry]
    private_compatible = filter_by_agent(private_candidates, target_agent)

    if len(private_compatible) == 1:
        resolved = candidate_view(rank_candidates(private_compatible, default_registry=default_registry, target_agent=target_agent, query=query)[0])
        return {
            'ok': True,
            'query': query,
    ...
    if len(private_compatible) > 1:
        ranked = rank_candidates(private_compatible, default_registry=default_registry, target_agent=target_agent, query=query)
        return {
            'ok': True,
            'query': query,
            'state': 'ambiguous',
    ...
    external_candidates = [item for item in all_candidates if item.get('source_registry') != default_registry]
    external_compatible = filter_by_agent(external_candidates, target_agent)

    if len(external_compatible) == 1:
        resolved = candidate_view(rank_candidates(external_compatible, default_registry=default_registry, target_agent=target_agent, query=query)[0])
        return {
            'ok': True,
            'query': query,
    ...
    if len(external_compatible) > 1:
        ranked = rank_candidates(external_compatible, default_registry=default_registry, target_agent=target_agent, query=query)
        return {
            'ok': True,
            'query': query,
            'state': 'ambiguous',
    ...
    if all_candidates:
        ranked = rank_candidates(all_candidates, default_registry=default_registry, target_agent=target_agent, query=query)
        return {
            'ok': True,
            'query': query,
            'state': 'incompatible',
