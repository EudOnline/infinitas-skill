...
def load_helpers():
    spec = importlib.util.spec_from_file_location(
        "test_attestation_verification_helpers", HELPER_PATH
    )
...
def scenario_ci_payload_contains_release_and_ci_context():
    ...
    try:
        ...
        if payload.get("attestation", {}).get("format") != "ci":
            fail(
                f"expected attestation.format 'ci', got {payload.get('attestation', {}).get('format')!r}"
            )
        ...
        if (distribution.get("bundle") or {}).get("path") != str(bundle_path.relative_to(repo)):
            fail(
                f"unexpected distribution bundle path {(distribution.get('bundle') or {}).get('path')!r}"
            )
    finally:
