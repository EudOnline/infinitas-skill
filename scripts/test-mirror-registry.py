...
def scenario_mirror_helper_and_ops_docs():
    ...
    try:
        ...
        missing_remote = run(
            [str(mirror_script), "--remote", "missing-remote", "--dry-run"], cwd=repo, expect=1
        )
        assert_contains(
            missing_remote.stderr + missing_remote.stdout, "missing remote", "missing remote error"
        )

        (repo / "DIRTY.txt").write_text("dirty\n", encoding="utf-8")
        dirty = run(
            [str(mirror_script), "--remote", "github-mirror", "--dry-run"], cwd=repo, expect=1
        )
        assert_contains(dirty.stderr + dirty.stdout, "dirty", "dirty tree error")
        (repo / "DIRTY.txt").unlink()

        reverse = run(
            [str(mirror_script), "--remote", "github-mirror", "--dry-run", "--fetch"],
            cwd=repo,
            expect=1,
        )
        assert_contains(reverse.stderr + reverse.stdout, "reverse sync", "reverse sync rejection")

        deployment_doc = (repo / "docs" / "ops" / "server-deployment.md").read_text(
            encoding="utf-8"
        )
        backup_doc = (repo / "docs" / "ops" / "server-backup-and-restore.md").read_text(
            encoding="utf-8"
        )
