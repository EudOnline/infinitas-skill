...
def scenario_snapshot_metadata_is_visible_to_catalog_and_registry_listing():
    ...
    try:
        ...
        registries_payload = json.loads(
            (repo / "catalog" / "registries.json").read_text(encoding="utf-8")
        )
        ...
        if f"latest_snapshot={snapshot_id}" not in combined:
            fail(
                f"expected registry listing to mention latest snapshot {snapshot_id!r}, got {combined!r}"
            )
    finally:
...
def scenario_create_snapshot_from_synced_remote_registry():
    ...
    try:
        ...
        created = run(
            ["python3", "scripts/create-registry-snapshot.py", "upstream", "--json"], cwd=repo
        )
        ...
        if refresh_state.get("source_commit") != expected_commit:
            fail(
                f"expected copied refresh_state source_commit {expected_commit!r}, got {metadata!r}"
            )
    finally:
        shutil.rmtree(tmpdir)
...
def scenario_create_snapshot_rejects_local_only_registry():
    ...
    try:
        repo = copy_repo(tmpdir)
        failed = run(
            ["python3", "scripts/create-registry-snapshot.py", "self", "--json"], cwd=repo, expect=1
        )
...
def scenario_resolve_install_and_sync_can_use_explicit_snapshot():
    ...
    try:
        ...
        if payload.get("skill_path") != str(
            (snapshot_root / "skills" / "active" / "demo").resolve()
        ):
        ...
        synced = run(
            ["bash", "scripts/sync-registry-source.sh", "upstream", "--snapshot", snapshot_id],
            cwd=repo,
        )
        synced_path = synced.stdout.strip()
        if synced_path != str(snapshot_root.resolve()):
            fail(
                f"expected snapshot sync to print {snapshot_root.resolve()!s}, got {synced_path!r}"
            )
    finally:
        shutil.rmtree(tmpdir)
