...
FIXTURE_NAME = 'server-hook-skill'
...
def fixture_skill_files():
    meta = {
        ...
        'status': 'incubating',
        'summary': 'Hosted publish hook fixture',
        'risk_level': 'low',
        'owner': 'release-owner',
    ...
    return {
        'SKILL.md': (
            '---\n'
            f'name: {FIXTURE_NAME}\n'
            'description: Hosted publish hook fixture.\n'
            '---\n\n'
            '# Hosted Publish Hook Fixture\n\n'
            'Used only by automated hosted publish-hook tests.\n'
        ),
        '_meta.json': meta,
        'CHANGELOG.md': (
            '# Changelog\n\n'
            f'## {FIXTURE_VERSION} - 2026-03-14\n'
            '- Added hosted publish hook fixture.\n'
        ),
        'tests/smoke.md': '# Smoke\n\n- Confirm the publish hook fixture releases cleanly.\n',
    }
...
def prepare_repo():
    tmpdir = Path(tempfile.mkdtemp(prefix='infinitas-publish-hook-test-'))
    repo = tmpdir / 'repo'
    ...
    run(['git', 'config', 'user.name', 'Release Hook Fixture'], cwd=repo)
    run(['git', 'config', 'user.email', 'release-hook@example.com'], cwd=repo)
    ...
    key_path = tmpdir / 'release-hook-key'
    run(
        [
            sys.executable,
    ...
    run(['git', 'push'], cwd=repo)
    return tmpdir, repo, origin, mirror, artifacts, key_path
...
def configure_env(tmpdir: Path, repo: Path, artifacts: Path, key_path: Path):
    ...
    os.environ['INFINITAS_SERVER_MIRROR_REMOTE'] = 'github-mirror'
    os.environ['INFINITAS_SERVER_MIRROR_BRANCH'] = 'main'


def assert_git_ref_exists(repo: Path, ref: str, label: str):
    result = subprocess.run(['git', '--git-dir', str(repo), 'show-ref', '--verify', ref], text=True, capture_output=True)
    if result.returncode != 0:
        fail(f'{label} missing {ref}\nstdout:\n{result.stdout}\nstderr:\n{result.stderr}')


def scenario_publish_runs_optional_mirror_hook():
    tmpdir, repo, _origin, mirror, artifacts, key_path = prepare_repo()
    try:
        ...
        create_response = client.post(
            ...
            json={
                ...
                'publisher': 'lvxiaoer',
                'payload_summary': 'Fixture submission for publish hook tests',
                'payload': {
        ...
        assert_git_ref_exists(mirror, 'refs/heads/main', 'mirror remote')
        ...
        log_text = job_response.json().get('log') or ''
        if 'mirror-registry.sh' not in log_text:
            fail(f'expected publish job log to include mirror-registry.sh, got:\n{log_text}')
        if 'github-mirror' not in log_text:
            fail(f'expected publish job log to include mirror remote, got:\n{log_text}')
    finally:
        shutil.rmtree(tmpdir, ignore_errors=True)


def main():
    scenario_publish_runs_optional_mirror_hook()
    print('OK: hosted publish hook checks passed')
