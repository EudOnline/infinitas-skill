...
def evaluate_refresh_status(root: Path, reg, *, now=None):
    ...
    has_policy = any(value is not None for value in [interval_hours, max_cache_age_hours, stale_policy])
    ...
    cache_path = None
    if isinstance(state, dict) and isinstance(state.get('cache_path'), str) and state.get('cache_path').strip():
