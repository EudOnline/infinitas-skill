    ...
    try:
        ...
        result = run(
            [
                sys.executable,
                str(repo / "scripts" / "review-status.py"),
                "invalid-platform-review-fixture",
                "--as-active",
                "--json",
            ],
            cwd=repo,
    ...
    try:
        ...
        result = run(
            [
                sys.executable,
                str(repo / "scripts" / "review-status.py"),
                "duplicate-platform-review-fixture",
                "--as-active",
                "--json",
            ],
            cwd=repo,
...
def scenario_import_cli_threads_platform_evidence_into_governance_surfaces():
    ...
    try:
        ...
        imported_latest = next(
            (
                item
                for item in evaluation.get("latest_decisions") or []
                if item.get("reviewer") == "bob"
            ),
            None,
        )
        if not imported_latest or imported_latest.get("source_kind") != "github-pull-review":
            fail(
                f"expected imported latest decision in promotion output, got {promotion_payload!r}"
            )
        ...
        release_latest = next(
            (
                item
                for item in ((release_payload.get("review") or {}).get("latest_decisions") or [])
                if item.get("reviewer") == "bob"
            ),
            None,
        )
        if not release_latest or release_latest.get("source_ref") != "pr#84":
            fail(
                f"expected imported latest decision in release-state output, got {release_payload!r}"
            )

        run(["bash", "scripts/build-catalog.sh"], cwd=repo)
        catalog_payload = json.loads(
            (repo / "catalog" / "catalog.json").read_text(encoding="utf-8")
        )
        catalog_item = next(
            (
                item
                for item in catalog_payload.get("skills") or []
                if item.get("name") == "import-platform-review-fixture"
            ),
            None,
        )
        if not catalog_item:
            fail(
                f"expected catalog entry for import-platform-review-fixture, got {catalog_payload!r}"
            )
        imported_reviewer = next(
            (item for item in catalog_item.get("reviewers") or [] if item.get("reviewer") == "bob"),
            None,
        )
...
def scenario_import_cli_rejects_missing_or_malformed_input():
    tmpdir = Path(
        tempfile.mkdtemp(prefix="infinitas-platform-review-evidence-import-invalid-test-")
    )
    try:
        repo = copy_repo(tmpdir)
        rewrite_policy(repo)
