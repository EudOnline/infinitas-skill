...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Generate a stable release attestation payload for a skill"
    )
...
def main():
    ...
    try:
        context = collect_release_context(
            args.skill, root=ROOT, releaser=args.releaser, release_mode=args.release_mode
        )
    ...
    output_name = (
        args.output_name or f"{context['meta'].get('name')}-{context['meta'].get('version')}.json"
    )
