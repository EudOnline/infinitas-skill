...
def configure_external_registry(repo: Path, tmpdir: Path):
    ...
    (
        ...
        / "catalog"
        / "provenance"
        / f"{EXTERNAL_SKILL_NAME}-{EXTERNAL_SKILL_VERSION}.json"
    ).write_text("{}\n", encoding="utf-8")
...
def assert_explanation(payload, *, registry_name, requires_confirmation, expected_version):
    ...
    if (
        not isinstance(explanation.get("selection_reason"), str)
        or not explanation.get("selection_reason").strip()
    ):
    ...
    if not isinstance(
        explanation.get("version_reason"), str
    ) or expected_version not in explanation.get("version_reason"):
        fail(
            f"expected version_reason to mention {expected_version!r}, got {explanation.get('version_reason')!r}"
        )
...
def main():
    ...
    try:
        ...
        assert_explanation(
            private_payload, registry_name="self", requires_confirmation=False, expected_version=V1
        )
        ...
        if no_upgrade_plan_payload.get("next_step") != "use-installed-skill":
            fail(
                f"expected confirm no-op next_step 'use-installed-skill', got {no_upgrade_plan_payload!r}"
            )
        assert_mutation_fields(
            no_upgrade_plan_payload,
            readiness="ready",
            policy=None,
            reason_code=None,
        ...
        assert_explanation(
            update_payload, registry_name="self", requires_confirmation=False, expected_version=V2
        )
        ...
        assert_explanation(
            plan_payload, registry_name="self", requires_confirmation=False, expected_version=V2
        )
        assert_mutation_fields(
            plan_payload, readiness="ready", policy=None, reason_code=None, recovery_action="none"
        )
        ...
        if stale_no_upgrade_payload.get("state") != "up-to-date":
            fail(
                f"expected stale confirm no-op state 'up-to-date', got {stale_no_upgrade_payload!r}"
            )
        if stale_no_upgrade_payload.get("freshness_state") != "stale":
            fail(
                f"expected stale confirm no-op freshness_state 'stale', got {stale_no_upgrade_payload!r}"
            )
        assert_mutation_fields(
        ...
        assert_explanation(
            never_verified_update_payload,
            registry_name="self",
            requires_confirmation=False,
            expected_version=V2,
        )
        never_verified_update_reasons = (
            never_verified_update_payload.get("explanation") or {}
        ).get("policy_reasons") or []
        if not any("never-verified" in reason.lower() for reason in never_verified_update_reasons):
            fail(
                f"expected never-verified policy reason in update explanation, got {never_verified_update_reasons!r}"
            )
        never_verified_update_actions = (
            never_verified_update_payload.get("explanation") or {}
        ).get("next_actions") or []
        if not any(
            "report-installed-integrity.py" in action and "--refresh" in action
            for action in never_verified_update_actions
        ):
            fail(
        ...
        never_verified_plan_reasons = (never_verified_plan_payload.get("explanation") or {}).get(
            "policy_reasons"
        ) or []
        if not any("never-verified" in reason.lower() for reason in never_verified_plan_reasons):
            fail(
                f"expected never-verified policy reason in confirm explanation, got {never_verified_plan_reasons!r}"
            )
        never_verified_plan_actions = (never_verified_plan_payload.get("explanation") or {}).get(
            "next_actions"
        ) or []
        if not any(
            "report-installed-integrity.py" in action and "--refresh" in action
            for action in never_verified_plan_actions
        ):
            fail(
        ...
        stale_warn_reasons = (stale_warn_payload.get("explanation") or {}).get(
            "policy_reasons"
        ) or []
        ...
        if not any("report-installed-integrity.py" in action for action in stale_warn_actions):
            fail(
                f"expected stale warning next_actions to recommend refresh, got {stale_warn_actions!r}"
            )
