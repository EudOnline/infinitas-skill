...
LEGACY_MANIFEST_REL = Path(
    "catalog/distributions/lvxiaoer/operate-infinitas-skill/0.1.1/manifest.json"
)
TRUST_CONFIG_REFS = [
...
def scenario_legacy_manifest_backfill_is_additive_and_idempotent():
    ...
    try:
        ...
        if not isinstance(file_manifest, list) or not file_manifest:
            fail(
                f"expected rewritten manifest to include non-empty file_manifest, got {file_manifest!r}"
            )
        ...
        immutable_checks = [
            (
                "bundle.path",
                (rewritten.get("bundle") or {}).get("path"),
                (original.get("bundle") or {}).get("path"),
            ),
            (
                "bundle.sha256",
                (rewritten.get("bundle") or {}).get("sha256"),
                (original.get("bundle") or {}).get("sha256"),
            ),
            (
        ...
        for label, actual, expected in immutable_checks:
            if actual != expected:
                fail(
                    f"expected {label} to stay unchanged\nactual: {actual!r}\nexpected: {expected!r}"
                )
...
def scenario_scan_mode_reports_machine_readable_status_per_manifest():
    ...
    try:
        ...
        broken_manifest_path = (
            tmpdir
            / "catalog/distributions/lvxiaoer/operate-infinitas-skill-broken/0.1.1/manifest.json"
        )
        broken_manifest_path.parent.mkdir(parents=True, exist_ok=True)
        broken_payload = load_json(manifest_path)
        broken_payload["bundle"] = {
            **(broken_payload.get("bundle") or {}),
            "path": "catalog/distributions/missing/skill.tar.gz",
        }
        ...
        if canonical_manifest not in by_manifest:
            fail(
                f"expected scan results to include canonical fixture manifest, got {by_manifest!r}"
            )
        ...
        if canonical_status.get("installed_integrity_capability") != "unknown":
            fail(
                f"expected canonical installed_integrity_capability 'unknown', got {canonical_status!r}"
            )
        if canonical_status.get("installed_integrity_reason") != "missing-signed-file-manifest":
            fail(
                f"expected canonical installed_integrity_reason 'missing-signed-file-manifest', got {canonical_status!r}"
            )
        ...
        if broken_status.get("installed_integrity_reason") != "missing-signed-file-manifest":
            fail(
                f"expected broken installed_integrity_reason 'missing-signed-file-manifest', got {broken_status!r}"
            )
        ...
        if canonical_write.get("file_manifest_count", 0) < 1:
            fail(
                f"expected canonical write file_manifest_count to be positive, got {canonical_write!r}"
            )
...
def scenario_scan_mode_uses_scanned_repo_trust_config():
    ...
    try:
        ...
        if "has no signer entries" not in (canonical_status.get("error") or ""):
            fail(
                f"expected canonical scan error to mention missing trusted signers, got {canonical_status!r}"
            )
        if canonical_status.get("root") != str(tmpdir.resolve()):
            fail(
                f"expected canonical scan root {tmpdir.resolve()!s}, got {canonical_status.get('root')!r}"
            )
    finally:
        shutil.rmtree(tmpdir)
...
def scenario_placeholder_metadata_is_backfillable_and_dry_run_is_non_mutating():
    ...
    try:
        ...
        rewritten = load_json(manifest_path)
        if not isinstance(rewritten.get("file_manifest"), list) or not rewritten.get(
            "file_manifest"
        ):
            fail(
                f"expected file_manifest list to be backfilled from placeholders, got {rewritten.get('file_manifest')!r}"
            )
        build = rewritten.get("build")
        if not isinstance(build, dict) or build.get("archive_format") != "tar.gz":
