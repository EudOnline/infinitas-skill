...
def main():
    ...
    discovery_entries = {
        item.get("qualified_name"): item
        for item in discovery_index.get("skills") or []
        if isinstance(item, dict)
    }
    compatibility_entries = {
        item.get("qualified_name"): item
        for item in compatibility.get("skills") or []
        if isinstance(item, dict)
    }
    ...
    for qualified_name, expected in EXPECTED_SKILLS.items():
        ...
        for label, entry in [("ai-index", ai_entry), ("discovery-index", discovery_entry)]:
            ...
            if not isinstance(entry.get("verified_support"), dict) or not entry.get(
                "verified_support"
            ):
        ...
        if missing_platforms:
            fail(
                f"compatibility {qualified_name} missing evidence for {sorted(missing_platforms)!r}"
            )
