...
def run_report(repo: Path, target: Path, *, expect=0):
    ...
    try:
        ...
        fail(
            f"report-installed-integrity.py did not emit JSON:\n{result.stdout}\n{result.stderr}\n{exc}"
        )
...
def main():
    ...
    try:
        ...
        if not isinstance(plan.get("steps"), list):
            fail(
                f"expected dependency planner to accept legacy manifest\n{result.stdout}\n{result.stderr}"
            )
        ...
        if item.get("integrity_capability") != "unknown":
            fail(
                f"expected report integrity_capability 'unknown' for legacy manifest, got {item!r}"
            )
        ...
        if item.get("freshness_state") != "never-verified":
            fail(
                f"expected report freshness_state 'never-verified' for legacy manifest, got {item!r}"
            )
        ...
        if item.get("mutation_reason_code") != "never-verified-installed-integrity":
            fail(
                f"expected legacy report mutation_reason_code 'never-verified-installed-integrity', got {item!r}"
            )
        if item.get("recovery_action") != "reinstall":
            fail(f"expected legacy report recovery_action 'reinstall', got {item!r}")
        if not isinstance(item.get("recommended_action"), str) or not item.get(
            "recommended_action"
        ):
        ...
        result = run(
            [
                str(repo / "scripts" / "update-install-manifest.py"),
                str(target),
                str(target / "demo-skill"),
                str(target / "demo-skill"),
                "sync",
                "1.2.3",
                json.dumps({"registry_name": "self"}),
            ],
            cwd=repo,
        )
        combined = result.stdout + result.stderr
        ...
        if payload.get("schema_version") != 1:
            fail(
                f"expected rewritten manifest to declare schema_version=1, got {payload.get('schema_version')!r}"
            )
        ...
        if integrity.get("state") != "unknown":
            fail(
                f"expected rewritten manifest integrity state 'unknown', got {integrity.get('state')!r}"
            )
        if current.get("integrity_capability") != "unknown":
            fail(
                f"expected rewritten manifest integrity_capability 'unknown', got {current.get('integrity_capability')!r}"
            )
        if current.get("integrity_reason") is not None:
            fail(
                f"expected rewritten manifest integrity_reason null, got {current.get('integrity_reason')!r}"
            )
        if not isinstance(current.get("last_checked_at"), str) or not current.get(
            "last_checked_at"
        ):
