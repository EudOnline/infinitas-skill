...
def report_map(payload):
    ...
    return {
        item.get("name") or item.get("qualified_name"): item
        for item in skills
...
def main():
    ...
    try:
        ...
        if (
            not isinstance(fresh.get("checked_age_seconds"), int)
            or fresh.get("checked_age_seconds") < 0
        ):
        ...
        if legacy.get("mutation_reason_code") != "never-verified-installed-integrity":
            fail(
                f"expected legacy mutation_reason_code 'never-verified-installed-integrity', got {legacy!r}"
            )
        ...
        if drifted.get("mutation_reason_code") != "drifted-installed-skill":
            fail(
                f"expected drifted mutation_reason_code 'drifted-installed-skill', got {drifted!r}"
            )
