...
def scenario_report_refresh_captures_drift_and_repair_history():
    ...
    try:
        ...
        if initial_item.get("qualified_name") != FIXTURE_NAME:
            fail(
                f"expected qualified_name {FIXTURE_NAME!r}, got {initial_item.get('qualified_name')!r}"
            )
        if initial_item.get("installed_version") != VERSION:
            fail(
                f"expected installed_version {VERSION!r}, got {initial_item.get('installed_version')!r}"
            )
        ...
        if (
            not isinstance(initial_item.get("checked_age_seconds"), int)
            or initial_item.get("checked_age_seconds") < 0
        ):
            fail(f"expected initial checked_age_seconds integer, got {initial_item!r}")
        if not isinstance(initial_item.get("last_checked_at"), str) or not initial_item.get(
            "last_checked_at"
        ):
        ...
        if not initial_events:
            fail(
                f"expected install report to surface at least one integrity event, got {initial_item!r}"
            )
        ...
        if drift_item.get("mutation_reason_code") != "drifted-installed-skill":
            fail(
                f"expected drifted mutation_reason_code 'drifted-installed-skill', got {drift_item!r}"
            )
        ...
        if manifest_item.get("integrity_events") != drift_events:
            fail(
                f"expected refreshed manifest to persist drift event history, got {manifest_item!r}"
            )
        ...
        if len(repaired_events) <= len(drift_events):
            fail(
                f"expected repaired refresh to append a later integrity event, got {repaired_item!r}"
            )
        last_repaired_event = repaired_events[-1] if repaired_events else {}
        if last_repaired_event.get("event") not in {"verified", "repaired"}:
            fail(
                f"expected final integrity event to be 'verified' or 'repaired', got {repaired_item!r}"
            )
        if last_repaired_event.get("source") not in {"refresh", "repair"}:
            fail(
                f"expected final integrity event source 'refresh' or 'repair', got {repaired_item!r}"
            )
    finally:
