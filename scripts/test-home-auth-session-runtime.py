...
def run_playwright(
    session: str, *args: str, check: bool = True, timeout: float | None = 120
) -> str:
...
def wait_for_eval(
    session: str, script: str, predicate, description: str, timeout: float = 10.0
) -> dict:
...
def scenario_home_waits_for_explicit_auth_before_probng_session():
    ...
    try:
        ...
        with HostedAppServer(ROOT, env) as server:
            ...
            if (
                anonymous_state.get("loginHidden") is not False
                or anonymous_state.get("loggedHidden") is not True
            ):
                fail(
                    f"expected anonymous home load to keep the login panel visible, got {anonymous_state}"
                )
            if (
                anonymous_state.get("cookieHint") is not False
                or anonymous_state.get("bootstrappedUser") is not None
            ):
                fail(
                    f"expected anonymous APP_SESSION bootstrap to stay empty, got {anonymous_state}"
                )
    finally:
        stop_playwright_session(session)
    ...
    try:
        ...
        with HostedAppServer(ROOT, env) as server:
            ...
            if login_state.get("cookieVisible") is not False:
                fail(
                    f"expected browser auth cookie to stay HttpOnly after login, got {login_state}"
                )
            if login_state.get("localToken") is not None:
                fail(
                    f"expected login flow to avoid storing raw auth token in localStorage, got {login_state}"
                )
            if (
                login_state.get("cookieHint") is not True
                or login_state.get("bootstrappedUser") != "fixture-maintainer"
            ):
                fail(
                    f"expected login redirect to hydrate APP_SESSION with current user, got {login_state}"
                )
            ...
            if cleared.get("cookieVisible") is not False:
                fail(
                    f"expected browser auth cookie to remain hidden after clearing localStorage, got {cleared}"
                )
            ...
            if session_state.get("icon") != "👤":
                fail(
                    f"expected home page to show logged-in icon after cookie-only reload, got {session_state}"
                )
    finally:
        stop_playwright_session(session)
        shutil.rmtree(tmpdir, ignore_errors=True)
    ...
    try:
        ...
        with HostedAppServer(ROOT, env) as server:
            ...
            if "Unauthorized" in reload_output or "/api/auth/me" in reload_output:
                fail(
                    f"expected stale cookie probe to avoid unauthorized console noise, got:\n{reload_output}"
                )
    ...
    try:
        ...
        with HostedAppServer(ROOT, env) as server:
            ...
            if empty_state.get("tagName") != "BUTTON":
                fail(
                    f"search empty state CTA should be a button instead of a dead link, got {empty_state}"
                )
            if empty_state.get("href") is not None:
                fail(
                    f"search empty state CTA should not navigate to another route, got {empty_state}"
                )
            ...
            if search_skill.get("tagName") != "BUTTON":
                fail(
                    f"search skill result should be a button action instead of a dead link, got {search_skill}"
                )
            if search_skill.get("href") is not None:
                fail(
                    f"search skill result should not navigate to a missing route, got {search_skill}"
                )
            if search_skill.get("copy"):
                if not search_skill["copy"].startswith("uv run infinitas discovery inspect "):
                    fail(
                        f"search skill result should expose an inspect command copy action, got {search_skill}"
                    )
            else:
                if search_skill.get("dropdownRole") != "dialog" or not search_skill.get(
                    "panelOpen"
                ):
                    fail(
                        f"search skill result should open the install detail panel, got {search_skill}"
                    )
                if int(search_skill.get("panelActionCount") or 0) <= 0:
                    fail(
                        f"search install detail panel should expose follow-up actions, got {search_skill}"
                    )
    finally:
        stop_playwright_session(session)
        shutil.rmtree(tmpdir, ignore_errors=True)
    ...
    try:
        ...
        with HostedAppServer(ROOT, env) as server:
            ...
            if english_runtime_copy.get("triggerAria") != "Sign in":
                fail(
                    f"expected english home trigger aria-label to stay english, got {english_runtime_copy}"
                )
            if english_runtime_copy.get("toggleAria") not in {
                ...
                fail(
                    f"expected english password toggle label to stay english, got {english_runtime_copy}"
                )
    ...
    try:
        ...
        with HostedAppServer(ROOT, env) as server:
            ...
            public_search = wait_for_eval(
                ...
                lambda result: (
                    result.get("dropdownHidden") is False and result.get("resultTag") == "BUTTON"
                ),
                "legacy registry reader env should not gate home search",
            )
            if public_search.get("modalHidden") is False:
                fail(
                    f"expected legacy registry reader env to stop triggering auth prompts on home search, got {public_search}"
                )
            if public_search.get("bodyOverflow"):
                fail(f"expected home search to keep body scroll unlocked, got {public_search}")
            if not (public_search.get("resultCopy") or "").startswith(
                "uv run infinitas discovery inspect "
            ):
                fail(
                    f"expected legacy registry reader env to preserve home inspect actions, got {public_search}"
                )
    finally:
        stop_playwright_session(session)
        shutil.rmtree(tmpdir, ignore_errors=True)
    ...
    try:
        ...
        with HostedAppServer(ROOT, env) as server:
            ...
            if protected_redirect.get("tokenInput") is not True:
                fail(
                    f"expected protected route redirect to expose the home auth modal, got {protected_redirect}"
                )
            if protected_redirect.get("search") not in ("", "?lang=en"):
                fail(
                    f"expected protected route redirect to clean up auth query params after boot, got {protected_redirect}"
                )
    finally:
        stop_playwright_session(session)
        shutil.rmtree(tmpdir, ignore_errors=True)
    ...
    try:
        ...
        with HostedAppServer(ROOT, env) as server:
            ...
            if login_state.get("ok") is not True or login_state.get("success") is not True:
                fail(
                    f"expected browser login bootstrap to succeed before console audit, got {login_state}"
                )
            ...
            if reauth_state.get("hasTokenInput") is not True:
                fail(
                    f"expected console reauth flow to expose the shared auth modal input, got {reauth_state}"
                )
