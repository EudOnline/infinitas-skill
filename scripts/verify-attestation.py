...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Verify a release attestation against repo-managed SSH and CI policy"
    )
    parser.add_argument("provenance", help="Path to the attestation JSON file")
    parser.add_argument(
        "--identity", help="Override signer identity (defaults to attestation.signer_identity)"
    )
...
def main():
    ...
    if args.json:
        ...
        print(
            f"OK: verified {args.provenance} with signer={result['identity']} namespace={result['namespace']}"
        )
