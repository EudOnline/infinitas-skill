...
def scenario_search_returns_trust_and_compatibility_fields():
    ...
    if match.get("runtime_assumptions") != [
        ...
        fail(
            f"expected canonical search runtime_assumptions, got {match.get('runtime_assumptions')!r}"
        )
...
def scenario_inspect_returns_distribution_and_dependency_views():
    ...
    if payload.get("qualified_name") != QUALIFIED_NAME:
        fail(
            f"expected inspect qualified_name {QUALIFIED_NAME!r}, got {payload.get('qualified_name')!r}"
        )
    for key in [
        "compatibility",
        "dependencies",
    ...
    if freshness_summary.get("codex") not in {"fresh", "stale", "unknown"}:
        fail(
            f"expected codex freshness summary fresh/stale/unknown, got {freshness_summary.get('codex')!r}"
        )
    dependencies = payload.get("dependencies") or {}
    summary = dependencies.get("summary") or {}
    if summary.get("root_name") != "operate-infinitas-skill":
        fail(
            f"expected dependency root_name 'operate-infinitas-skill', got {summary.get('root_name')!r}"
        )
    if summary.get("root_source_type") != "working-tree":
        fail(
            f"expected dependency root_source_type 'working-tree', got {summary.get('root_source_type')!r}"
        )
    ...
    if provenance.get("required_attestation_formats") != ["ssh"]:
        fail(
            f"expected required_attestation_formats ['ssh'], got {provenance.get('required_attestation_formats')!r}"
        )
    ...
    if distribution.get("source_type") != "distribution-manifest":
        fail(
            f"expected distribution source_type 'distribution-manifest', got {distribution.get('source_type')!r}"
        )
    ...
    if trust.get("signature_present") is not True:
        fail(
            f"expected inspect trust.signature_present true, got {trust.get('signature_present')!r}"
        )
    ...
    if decision_metadata.get("use_when") != [
        ...
        fail(
            f"expected inspect decision_metadata.use_when, got {decision_metadata.get('use_when')!r}"
        )
    if decision_metadata.get("avoid_when") != [
        "Need a general-purpose Git helper outside this repository"
    ]:
        fail(
            f"expected inspect decision_metadata.avoid_when, got {decision_metadata.get('avoid_when')!r}"
        )
    if decision_metadata.get("capabilities") != [
        "repo-operations",
        "release-guidance",
        "registry-debugging",
    ]:
        fail(
            f"expected inspect decision_metadata.capabilities, got {decision_metadata.get('capabilities')!r}"
        )
    ...
    if decision_metadata.get("maturity") != "stable":
        fail(
            f"expected inspect decision_metadata.maturity 'stable', got {decision_metadata.get('maturity')!r}"
        )
    if decision_metadata.get("quality_score") != 90:
        fail(
            f"expected inspect decision_metadata.quality_score 90, got {decision_metadata.get('quality_score')!r}"
        )
