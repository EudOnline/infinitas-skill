...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Rehearse restoring a hosted registry backup into a staging directory"
    )
    parser.add_argument(
        "--backup-dir", required=True, help="Backup directory created by `infinitas server backup`"
    )
    parser.add_argument(
        "--output-dir", required=True, help="Staging directory for the restore rehearsal"
    )
...
def main():
    ...
    if not repo_bundle_name or not db_backup_name or not artifacts_archive_name:
        fail(
            f"manifest.json is missing required backup file references: {backup_dir / 'manifest.json'}"
        )
