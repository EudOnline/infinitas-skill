...
def load_current_skill_payload():
    ...
    distributions = json.loads(
        (ROOT / "catalog" / "distributions.json").read_text(encoding="utf-8")
    )
    skill = next(
        item for item in ai_index.get("skills", []) if item.get("qualified_name") == QUALIFIED_NAME
    )
    dist = next(
        item
        for item in distributions.get("skills", [])
...
def prepare_served_registry(
    ...
    *,
    ...
    write_json(
        root_dir / "distributions.json",
        {"generated_at": "2026-03-13T00:00:00Z", "skills": [dist_entry]},
    )
    write_json(
        root_dir / "compatibility.json", {"generated_at": "2026-03-13T00:00:00Z", "skills": []}
    )
...
def assert_install_success(repo: Path, target_dir: Path, *, expected_integrity_state: str):
    ...
    if not isinstance(entry.get("source_repo"), str) or not entry["source_repo"].startswith(
        "http://127.0.0.1:"
    ):
        fail(f"expected hosted source_repo URL, got {entry.get('source_repo')!r}")
    if (
        entry.get("source_distribution_manifest")
        != f"catalog/distributions/{PUBLISHER}/{SKILL_NAME}/{VERSION}/manifest.json"
    ):
        fail(
            f"unexpected source_distribution_manifest {entry.get('source_distribution_manifest')!r}"
        )
    if expected_integrity_state == "verified":
        ...
        if not source_distribution_root.is_dir():
            fail(
                f"expected hosted install to persist source_distribution_root, got {entry.get('source_distribution_root')!r}"
            )
        ...
        for rel in required_cached_refs:
            if not isinstance(rel, str) or not rel:
                fail(
                    f"expected cached hosted install references to be populated, got {required_cached_refs!r}"
                )
            if not (source_distribution_root / rel).exists():
                fail(
                    f"expected cached hosted install artifact to exist: {(source_distribution_root / rel)}"
                )
    ...
    if expected_integrity_state == "unknown" and integrity.get("last_verified_at") is not None:
        fail(
            f"expected hosted install integrity last_verified_at to stay null, got {integrity.get('last_verified_at')!r}"
        )
    if expected_integrity_state == "verified" and not integrity.get("last_verified_at"):
        fail(
            f"expected hosted install integrity last_verified_at to be populated, got {integrity.get('last_verified_at')!r}"
        )
    ...
    if expected_integrity_state == "unknown":
        if verify_payload.get("state") != "failed":
            fail(
                f"expected hosted explicit verification failure, got {verify_payload.get('state')!r}"
            )
        error = verify_payload.get("error") or ""
        if "missing signed file_manifest" not in error:
            fail(
                f"expected explicit verification to report missing signed file_manifest\n{verify.stdout}\n{verify.stderr}"
            )
    else:
        if verify_payload.get("state") != "verified":
            fail(
                f"expected hosted explicit verification success, got {verify_payload.get('state')!r}"
            )
    ...
    try:
        ...
        with HostedRegistryServer(success_root) as server:
            ...
            assert_install_success(
                repo, tmpdir / "installed-success", expected_integrity_state="unknown"
            )
        ...
        with HostedRegistryServer(backfilled_root) as server:
            ...
            assert_install_success(
                repo, tmpdir / "installed-backfilled-success", expected_integrity_state="verified"
            )
        ...
        with HostedRegistryServer(uninstalled_root) as server:
            ...
            assert_install_failure(
                repo, tmpdir / "installed-uninstallable", "version-not-installable"
            )
    finally:
