    ...
    try:
        ...
        for rel in PROFILE_PATHS:
            ...
            if runtime.get("entrypoint") != "SKILL.md":
                fail(
                    f"expected {rel} runtime.entrypoint to equal SKILL.md, got {runtime.get('entrypoint')!r}"
                )
            ...
            doc_payload, doc_errors = validate_platform_contract(
                repo / "docs" / "platform-contracts" / f"{platform}.md", REQUIRED_DOCS[platform]
            )
            if doc_errors:
                fail(f"expected {platform} contract doc to validate, got {doc_errors!r}")
            if sources != doc_payload.get("official_sources"):
                fail(
                    f"expected {rel} contract.sources to match docs/platform-contracts/{platform}.md, got {sources!r}"
                )
            expected_last_verified = (
                doc_payload.get("last_verified").isoformat()
                if doc_payload.get("last_verified")
                else None
            )
        ...
        matrix = (repo / "docs" / "reference" / "compatibility-matrix.md").read_text(
            encoding="utf-8"
        )
        contract_doc = (repo / "docs" / "reference" / "compatibility-contract.md").read_text(
            encoding="utf-8"
        )
        ...
        assert_contains(
            matrix, "declared support", "compatibility matrix declared support terminology"
        )
        assert_contains(
            matrix, "verified support", "compatibility matrix verified support terminology"
        )
        assert_contains(
            contract_doc, "declared support", "compatibility contract declared support terminology"
        )
        assert_contains(
            contract_doc, "verified support", "compatibility contract verified support terminology"
        )
