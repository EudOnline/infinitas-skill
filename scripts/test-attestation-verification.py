...
def scenario_release_notes_require_attestation():
    ...
    try:
        ...
        assert_contains(
            combined, "v9 attestation policy requires --write-provenance", "notes attestation gate"
        )
    finally:
...
def scenario_verified_attestation_bundle_is_emitted():
    ...
    try:
        ...
        distribution_dir = (
            repo / "catalog" / "distributions" / "_legacy" / FIXTURE_NAME / FIXTURE_VERSION
        )
        ...
        if provenance.get("skill", {}).get("author") != identity:
            fail(
                f"expected skill.author {identity!r}, got {provenance.get('skill', {}).get('author')!r}"
            )
        ...
        if "self" not in provenance.get("registry", {}).get("registries_consulted", []):
            fail(
                f"expected registry context to include self, got {provenance.get('registry', {}).get('registries_consulted')!r}"
            )
        ...
        if (distribution.get("bundle") or {}).get("path") != str(
            distribution_bundle.relative_to(repo)
        ):
        ...
        run(
            [str(repo / "scripts" / "verify-provenance-ssh.sh"), str(provenance_path)],
            cwd=repo,
            env=make_env(),
        )
        run(
...
def scenario_provenance_persists_delegated_audit_details():
    ...
    try:
        ...
        exception = next(
            (item for item in exception_usage if item.get("id") == "dirty-worktree-waiver"), None
        )
        if not exception:
...
def scenario_both_mode_requires_ssh_and_ci():
    ...
    try:
        ...
        signing_config_path.write_text(
            json.dumps(signing_config, ensure_ascii=False, indent=2) + "\n", encoding="utf-8"
        )
        result = run(
            [
                sys.executable,
        ...
        if payload.get("formats_verified") != ["ssh", "ci"]:
            fail(
                f"expected formats_verified ['ssh', 'ci'], got {payload.get('formats_verified')!r}"
            )
