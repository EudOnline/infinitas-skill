...
def run_in_repo(repo, command, *, expect=None):
    result = subprocess.run(command, cwd=repo, text=True, capture_output=True, env=cli_env(repo))
    if expect is not None and result.returncode != expect:
        fail(
...
def scenario_promotion_policy_trace():
    command = [
        *infinitas_cli(ROOT, "policy", "check-promotion"),
        "--json",
        "--as-active",
        str(ACTIVE_SKILL),
    ]
...
def scenario_promotion_policy_failure_trace():
    ...
    try:
        ...
        if trace.get("decision") != "deny":
            fail(
                f"expected deny decision for failing promotion trace, got {trace.get('decision')!r}"
            )
...
def scenario_promotion_exception_trace():
    ...
    try:
        ...
        record = next(
            (item for item in exceptions if item.get("id") == "promotion-trace-waiver"), None
        )
        if not record:
...
def scenario_release_policy_trace():
    command = [
        *infinitas_cli(ROOT, "release", "check-state"),
        str(ACTIVE_SKILL),
        "--json",
    ]
    result = run(command)
...
def scenario_validation_policy_traces():
    ...
    if not any(
        trace.get("domain") == "namespace_policy" for trace in traces if isinstance(trace, dict)
    ):
