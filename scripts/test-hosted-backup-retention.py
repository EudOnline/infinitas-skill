...
def create_backup_dir(root: Path, name: str):
    ...
    (backup_dir / "manifest.json").write_text(
        json.dumps({"created_at": name.split("-")[0]}, ensure_ascii=False) + "\n", encoding="utf-8"
    )
    return backup_dir
...
def scenario_render_prune_units():
    ...
    try:
        ...
        assert_contains(
            prune_service_text, "Environment=PYTHONPATH=/srv/infinitas/repo/src", "prune service"
        )
        assert_contains(
            prune_service_text, "-m infinitas_skill.cli.main server prune-backups", "prune service"
        )
