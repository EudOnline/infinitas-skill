...
def release_current(repo: Path, version: str):
    ...
    manifest = (
        repo / "catalog" / "distributions" / "_legacy" / FIXTURE_NAME / version / "manifest.json"
    )
...
def scenario_install_switch_and_rollback_use_distribution_manifests():
    ...
    try:
        ...
        if dependency_root.get("name") != FIXTURE_NAME:
            fail(
                f"expected distribution dependency root {FIXTURE_NAME!r}, got {dependency_root.get('name')!r}"
            )
        ...
        if history[-1].get("version") != V2:
            fail(
                f"expected latest history entry to preserve {V2}, got {history[-1].get('version')!r}"
            )
...
def scenario_installed_distribution_can_be_repaired_after_drift():
    ...
    try:
        ...
        if verify_payload.get("state") != "verified":
            fail(
                f"expected installed distribution to verify cleanly, got {verify_payload.get('state')!r}"
            )
        ...
        if verify_payload.get("state") != "verified":
            fail(
                f"expected repaired distribution install to verify cleanly, got {verify_payload.get('state')!r}"
            )
    finally:
        shutil.rmtree(tmpdir)
...
def scenario_installed_distribution_verifies_after_legacy_manifest_backfill():
    ...
    try:
        ...
        if failed_verify.get("state") != "failed":
            fail(
                f"expected explicit verification to fail before backfill, got {failed_verify.get('state')!r}"
            )
        if "missing signed file_manifest" not in (failed_verify.get("error") or ""):
            fail(
                f"expected explicit verification to fail on missing signed file_manifest, got {failed_verify!r}"
            )
        ...
        if backfill_payload.get("state") not in {"backfilled", "unchanged"}:
            fail(
                f"expected backfill state 'backfilled' or 'unchanged', got {backfill_payload.get('state')!r}"
            )
        ...
        if verified_payload.get("state") != "verified":
            fail(
                f"expected explicit verification success after backfill, got {verified_payload.get('state')!r}"
            )
    finally:
        shutil.rmtree(tmpdir)
...
def scenario_switch_and_rollback_force_bypass_stale_guardrails_and_keep_drift_precedence():
    ...
    try:
        ...
        if (
            "verify-installed-skill.py" not in switch_drift_output
            or "repair-installed-skill.sh" not in switch_drift_output
        ):
            fail(
                f"expected switch drift precedence output to recommend verify+repair\n{switch_drift_output}"
            )
        if "report-installed-integrity.py" in switch_drift_output:
            fail(
                f"expected switch drift precedence to avoid stale refresh guidance\n{switch_drift_output}"
            )
        ...
        if (
            "verify-installed-skill.py" not in rollback_drift_output
            or "repair-installed-skill.sh" not in rollback_drift_output
        ):
            fail(
                f"expected rollback drift precedence output to recommend verify+repair\n{rollback_drift_output}"
            )
        if "report-installed-integrity.py" in rollback_drift_output:
            fail(
                f"expected rollback drift precedence to avoid stale refresh guidance\n{rollback_drift_output}"
            )
    finally:
        shutil.rmtree(tmpdir)
...
def scenario_switch_and_rollback_never_verified_guardrails_and_force_bypass():
    ...
    try:
        ...
        if (
            "report-installed-integrity.py" not in switch_warn_output
            or "--refresh" not in switch_warn_output
        ):
            fail(
                f"expected never-verified warn switch output to recommend refresh command\n{switch_warn_output}"
            )
        assert_current_install(target_dir, V1)
        ...
        if (
            "report-installed-integrity.py" not in rollback_warn_output
            or "--refresh" not in rollback_warn_output
        ):
            fail(
                f"expected never-verified warn rollback output to recommend refresh command\n{rollback_warn_output}"
            )
        assert_current_install(target_dir, V2)
        ...
        if (
            "report-installed-integrity.py" not in switch_fail_output
            or "--refresh" not in switch_fail_output
        ):
            fail(
                f"expected never-verified fail switch output to recommend refresh command\n{switch_fail_output}"
            )
        assert_current_install(target_dir, V2)
        ...
        if (
            "report-installed-integrity.py" not in rollback_fail_output
            or "--refresh" not in rollback_fail_output
        ):
            fail(
                f"expected never-verified fail rollback output to recommend refresh command\n{rollback_fail_output}"
            )
        assert_current_install(target_dir, V2)
        ...
        if (
            ...
            fail(
                f"expected never-verified switch drift precedence output to recommend verify+repair\n{switch_drift_output}"
            )
        if "report-installed-integrity.py" in switch_drift_output:
            fail(
                f"expected never-verified switch drift precedence to avoid refresh guidance\n{switch_drift_output}"
            )
        ...
        if (
            ...
            fail(
                f"expected never-verified rollback drift precedence output to recommend verify+repair\n{rollback_drift_output}"
            )
        if "report-installed-integrity.py" in rollback_drift_output:
            fail(
                f"expected never-verified rollback drift precedence to avoid refresh guidance\n{rollback_drift_output}"
            )
    finally:
        shutil.rmtree(tmpdir)
