...
def scenario_selector_smoke_test():
    return
...
def scenario_attestation_can_publish_to_transparency_log():
    ...
    try:
        ...
        with TransparencyServer("good") as server:
            ...
            if entry.get("attestation_sha256") != (server.requests[0].get("payload") or {}).get(
                "attestation_sha256"
            ):
                fail(
                    f"expected logged digest to match request payload, got {entry!r} vs {server.requests!r}"
                )
            request_payload = server.requests[0].get("payload") or {}
            if request_payload.get("skill", {}).get("name") != FIXTURE_NAME:
                fail(
                    f"expected transparency payload to include skill metadata, got {request_payload!r}"
                )
            if (
                request_payload.get("source_snapshot", {}).get("tag")
                != f"skill/{FIXTURE_NAME}/v{FIXTURE_VERSION}"
            ):
                fail(
                    f"expected transparency payload to include source snapshot tag, got {request_payload!r}"
                )
    finally:
    ...
    try:
        with TransparencyServer("good") as server:
            ...
            if transparency_summary.get("verified") is not True:
                fail(
                    f"expected verify-attestation to surface verified transparency summary, got {verified_payload!r}"
                )
            ...
            release_transparency = (release_payload.get("release") or {}).get(
                "transparency_log"
            ) or {}
            if release_transparency.get("verified") is not True:
                fail(
                    f"expected check-release-state JSON to surface transparency summary, got {release_payload!r}"
                )
            if release_transparency.get("entry_id") != "log-entry-1":
                fail(f"expected check-release-state entry_id, got {release_payload!r}")

            run(["bash", str(repo / "scripts" / "build-catalog.sh")], cwd=repo, env=make_env(repo))
            catalog = json.loads((repo / "catalog" / "catalog.json").read_text(encoding="utf-8"))
            catalog_item = next(
                (item for item in catalog.get("skills") or [] if item.get("name") == FIXTURE_NAME),
                None,
            )
            if not catalog_item:
                fail(f"expected catalog entry for {FIXTURE_NAME}, got {catalog!r}")
            catalog_transparency = (catalog_item.get("verified_distribution") or {}).get(
                "transparency_log"
            ) or {}
...
def scenario_required_transparency_mode_blocks_release_output():
    ...
    try:
        with TransparencyServer("mismatch") as server:
            ...
            provenance_path = (
                repo / "catalog" / "provenance" / f"{FIXTURE_NAME}-{FIXTURE_VERSION}.json"
            )
            if provenance_path.exists():
                fail(
                    f"expected required transparency failure to block provenance output, found {provenance_path}"
                )
            entry_path = (
                repo
                / "catalog"
                / "provenance"
                / f"{FIXTURE_NAME}-{FIXTURE_VERSION}.transparency.json"
            )
            if entry_path.exists():
                fail(
                    f"expected no transparency entry on failed required release, found {entry_path}"
                )
    finally:
        shutil.rmtree(tmpdir)
...
DEFAULT_SCENARIOS = {
    ...
    "scenario_required_mode_requires_endpoint": scenario_required_mode_requires_endpoint,
    "scenario_malformed_transparency_response_fails_clearly": (
        scenario_malformed_transparency_response_fails_clearly
    ),
    "scenario_proof_mismatch_fails_clearly": scenario_proof_mismatch_fails_clearly,
...
SELECTOR_ONLY_SCENARIOS = {
    "scenario_selector_smoke_test": scenario_selector_smoke_test,
}
SCENARIOS = {
    **DEFAULT_SCENARIOS,
    **SELECTOR_ONLY_SCENARIOS,
...
def main(argv: list[str] | None = None):
    ...
    scenario_names = selected or list(DEFAULT_SCENARIOS)
