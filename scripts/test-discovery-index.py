...
def assert_discovery_payload_stable_across_roots():
    ...
    primary = build_discovery_index(
        root=Path("/tmp/primary-repo"),
        local_ai_index=local_ai_index,
        registry_config=registry_config,
    )
    linked = build_discovery_index(
        root=Path("/tmp/primary-repo/.worktrees/linked"),
        local_ai_index=local_ai_index,
        registry_config=registry_config,
    )
    ...
    self_source = next(
        (item for item in primary.get("sources") or [] if item.get("name") == "self"), None
    )
    if not self_source:
    ...
    fixture = next(
        (item for item in primary.get("skills") or [] if item.get("name") == FIXTURE_NAME), None
    )
    if fixture is None:
    ...
    if fixture.get("attestation_formats") != ["ssh"]:
        fail(
            f"expected stable attestation_formats ['ssh'], got {fixture.get('attestation_formats')!r}"
        )
...
def main():
    ...
    try:
        ...
        if not isinstance(first.get("attestation_formats"), list) or not first.get(
            "attestation_formats"
        ):
        ...
        if first.get("last_verified_at") is not None and not isinstance(
            first.get("last_verified_at"), str
        ):
        ...
        fixture = next(
            (item for item in payload["skills"] if item.get("name") == FIXTURE_NAME), None
        )
        if fixture is None:
        ...
        external = next(
            (item for item in payload["skills"] if item.get("name") == EXTERNAL_SKILL_NAME), None
        )
        if external is None:
        ...
        if external.get("attestation_formats") != ["ssh"]:
            fail(
                f"expected external attestation_formats ['ssh'], got {external.get('attestation_formats')!r}"
            )
        ...
        if external.get("runtime_assumptions") != ["A trusted external registry is configured"]:
            fail(
                f"expected external runtime_assumptions, got {external.get('runtime_assumptions')!r}"
            )

        self_source = next(
            (item for item in payload.get("sources") or [] if item.get("name") == "self"), None
        )
        if not self_source:
            fail("expected discovery-index to include self source entry")
        if self_source.get("root") != ".":
