...
def scenario_namespace_team_authorization_passes():
    ...
    try:
        ...
        write_namespace_policy(repo, owner_teams=['platform-admins'], maintainer_teams=['platform-admins'])
        ...
        run([sys.executable, str(repo / 'scripts' / 'validate-registry.py'), str(skill_dir)], cwd=repo)
    finally:
...
def scenario_namespace_team_failure_is_structured():
    ...
    try:
        ...
        if not any('authorized owners' in message or 'authorized for publisher' in message for message in errors):
...
def scenario_team_review_scope_and_policy_trace():
    ...
    try:
        write_team_policy(repo)
        write_namespace_policy(repo, owner_teams=['release-owners'], maintainer_teams=['platform-admins'])
        ...
        run([str(repo / 'scripts' / 'request-review.sh'), 'delegated-review-fixture', '--note', 'Delegated team review'], cwd=repo)
        run([str(repo / 'scripts' / 'approve-skill.sh'), 'delegated-review-fixture', '--reviewer', 'alice', '--decision', 'approved'], cwd=repo)
        ...
        if fail_trace.get('decision') != 'deny':
            fail(f"expected deny decision before security review, got {fail_trace.get('decision')!r}")
        ...
        if not any('security' in json.dumps(item, ensure_ascii=False) for item in blocking_rules):
            fail(f'expected delegated review trace to mention missing security coverage, got {blocking_rules!r}')

        run([str(repo / 'scripts' / 'approve-skill.sh'), 'delegated-review-fixture', '--reviewer', 'bob', '--decision', 'approved'], cwd=repo)
        run(
            [
                sys.executable,
        ...
        if pass_trace.get('decision') != 'allow':
            fail(f"expected allow decision after delegated approvals, got {pass_trace.get('decision')!r}")
    finally:
        shutil.rmtree(tmpdir, ignore_errors=True)
