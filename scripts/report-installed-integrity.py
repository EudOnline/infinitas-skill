...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Report local installed-skill integrity state from the install manifest"
    )
    parser.add_argument("target_dir", help="Installed skills target directory")
    parser.add_argument(
        "--refresh",
        action="store_true",
        help="Re-run local integrity verification and persist refreshed summary fields",
    )
...
def main():
    ...
    if args.refresh:
        ...
        for name, item in skills.items():
            updated_skills[name] = _refresh_manifest_entry(
                target_dir, name, item, refreshed_at=refreshed_at
            )
