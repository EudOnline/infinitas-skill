...
def recommend_reviewers(skill_dir: Path, *, root: Path = ROOT, stage=None, as_active=False, policy=None):
    root = Path(root).resolve()
    skill_dir = Path(skill_dir).resolve()
    policy = policy or load_promotion_policy(root)
    evaluation = evaluate_review_state(skill_dir, root=root, stage=stage, as_active=as_active, policy=policy)
    groups, reviewers = configured_reviewers(policy, root=root)
    reviews_cfg = policy.get('reviews', {}) if isinstance(policy, dict) else {}
    quorum_rule = effective_quorum_rule(policy, evaluation.get('evaluated_stage'), evaluation.get('risk_level'))
    ...
    latest_by_reviewer = {item.get('reviewer'): item for item in evaluation.get('latest_decisions', []) if item.get('reviewer')}
    counted_reviewers = {item.get('reviewer') for item in evaluation.get('latest_decisions', []) if item.get('counted')}
    required_groups = list(evaluation.get('required_groups') or [])
    target_groups = list(evaluation.get('missing_groups') or [])
    approval_gap = max((evaluation.get('required_approvals') or 0) - (evaluation.get('approval_count') or 0), 0)
    ...
    for group_name in target_groups:
        ...
        for reviewer in group_data.get('resolved_members', []):
            reasons = []
            if reviewer == evaluation.get('owner') and reviews_cfg.get('reviewer_must_differ_from_owner') and not owner_fallback_allowed:
            ...
            escalations.append(
                {
                    ...
                    'message': f'No eligible reviewer is currently available for group {group_name}; update team membership or use a reviewed exception path.',
                }
            )
...
def render_reviewer_recommendations(payload):
    ...
    for group in payload.get('group_recommendations', []):
        ...
        if eligible:
            lines.append(f'recommended reviewers [{group_name}]: ' + ', '.join(item.get('reviewer') or '' for item in eligible))
        else:
